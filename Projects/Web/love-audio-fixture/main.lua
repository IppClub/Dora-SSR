...
function love.load()
	...
	clone = wav:clone()
	assert(wav:setFilter({type = "lowpass", highgain = 0.3}))
	assert(love.audio.setEffect("room", {type = "reverb", decaytime = 0.4, volume = 0.1}))
	assert(wav:setEffect("room"))
...
function love.update()
	...
	elseif frames == 8 then
		assert(wav:setFilter())
		assert(wav:setEffect("room", false))
		ogg:stop()
