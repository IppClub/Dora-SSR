...
function tl.search_module(module_name, search_dtl)
	...
	local path = package.path
	if search_dtl then
		found, tried = search_for(module_name, ".d.tl", path, tried)
		if found then
			return found
		end
	end
	found, tried = search_for(module_name, ".tl", path, tried)
	if found then
		return found
	end
	...
	a_type({ typename = "function", args = TUPLE({}), rets = TUPLE({ a, b }) }),
		...
		["io"] = a_type({
			...
			fields = {
				...
				["lines"] = a_type({ typename = "function", args = VARARG({ OPT_STRING, a_type({ opt = true, typename = "union", types = { STRING, NUMBER } }) }), rets = TUPLE({
					a_type({ typename = "function", args = TUPLE({}), rets = VARARG({ STRING }) }),
				}), }),
				["open"] = a_type({ typename = "function", args = TUPLE({ STRING, OPT_STRING }), rets = TUPLE({ NOMINAL_FILE, STRING }) }),
				["output"] = a_type({ typename = "function", args = TUPLE({ OPT_UNION({ STRING, NOMINAL_FILE }) }), rets = TUPLE({ NOMINAL_FILE }) }),
				["popen"] = a_type({ typename = "function", args = TUPLE({ STRING, OPT_STRING }), rets = TUPLE({ NOMINAL_FILE, STRING }) }),
				["read"] = a_type({ typename = "function", args = TUPLE({ OPT_UNION({ STRING, NUMBER }) }), rets = TUPLE({ STRING, STRING }) }),
				["stderr"] = NOMINAL_FILE,
				["stdin"] = NOMINAL_FILE,
				["stdout"] = NOMINAL_FILE,
			...
			fields = {
				...
				["date"] = a_type({
					...
					types = {
						...
						a_type({ typename = "function", args = TUPLE({ OS_DATE_TABLE_FORMAT, OPT_NUMBER }), rets = TUPLE({ OS_DATE_TABLE }) }),
...
tl.type_check = function(ast, opts)
	...
	local function match_fields_to_map(rec1, rec2)
		if not match_record_fields(rec1, function(_) return rec2.values end) then
		...
		elseif t1.typename == "function" and t2.typename == "function" then
			...
			if (not t2.args.is_va) and t1_min_arity > t2_min_arity then
				...
				if t1.is_method then
					local t2a = t2.args[1]
					if not t2a or not (t2a.typename == "nominal" or t2a.typename == "record" or t2a.typename == "arrayrecord") then
						table.insert(all_errs, error_in_type(t1, "argument 1 expects record or nominal"))
					end
				end
				if t2.is_method then
					local t1a = t1.args[1]
					if not t1a or not (t1a.typename == "nominal" or t1a.typename == "record" or t1a.typename == "arrayrecord") then
						table.insert(all_errs, error_in_type(t1, "argument 1 expects record or nominal"))
					end
				end
				local t1nargs = #t1.args
				local t2nargs = #t2.args
				for i = (t1.is_method or t2.is_method) and 2 or 1, t1nargs do
					local t1a = t1.args[i]
