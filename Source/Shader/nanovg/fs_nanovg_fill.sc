...
#define u_textSoftness (u_params.x)
#define u_textEdge     (u_params.y)
...
void main()
{
	...
	else if (u_type == 4.0) // SDF text
	{
		vec2 texSize = vec2(textureSize(s_tex, 0));
		vec2 texelSize = 1.0 / texSize;
		float sdfValue = (
			texture2D(s_tex, v_texcoord0.xy + vec2(-1, -1) * texelSize).a +
			texture2D(s_tex, v_texcoord0.xy + vec2( 0, -1) * texelSize).a +
			texture2D(s_tex, v_texcoord0.xy + vec2( 1, -1) * texelSize).a +
			texture2D(s_tex, v_texcoord0.xy + vec2(-1,  0) * texelSize).a +
			texture2D(s_tex, v_texcoord0.xy + vec2( 0,  0) * texelSize).a +
			texture2D(s_tex, v_texcoord0.xy + vec2( 1,  0) * texelSize).a +
			texture2D(s_tex, v_texcoord0.xy + vec2(-1,  1) * texelSize).a +
			texture2D(s_tex, v_texcoord0.xy + vec2( 0,  1) * texelSize).a +
			texture2D(s_tex, v_texcoord0.xy + vec2( 1,  1) * texelSize).a
		) / 9.0;
		float softness = max(u_textSoftness, 0.001);
		float alpha = smoothstep(u_textEdge - softness, u_textEdge + softness, sdfValue);
		result = u_innerCol * (alpha * scissor);
	}
