...
#include "wrap_GraphicsImageConstructor.h"
...
static GraphicsImageConstructorCommand *imageConstructorCommand(lua_State *L)
{
	auto *module = luax_getmodule(L, Module::M_GRAPHICS);
	auto *command = dynamic_cast<GraphicsImageConstructorCommand *>(module);
	if (!command) luaL_error(L, "love.graphics has no state-local Image factory");
	return command;
}

static love::image::Image *imageModule(lua_State *L)
{
	auto *module = luax_getmodule(L, Module::M_IMAGE);
	auto *image = dynamic_cast<love::image::Image *>(module);
	if (!image) luaL_error(L, "Cannot load images without the love.image module.");
	return image;
}
...
getImageData(lua_State *L, int idx, bool allowcompressed, float *dpiscale)
{
	...
	else if (filesystem::luax_cangetdata(L, idx))
	{
		...
		auto imagemodule = imageModule(L);
...
static int w__pushNewImage(lua_State *L, Image::Slices &slices, const Image::Settings &settings)
{
	StrongRef<Image> i;
	luax_catchexcept(L,
		[&]() { i.set(imageConstructorCommand(L)->newImage(slices, settings), Acquire::NORETAIN); },
		[&](bool) { slices.clear(); }
	);
