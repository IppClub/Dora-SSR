...
public:
	...
	// Dora hosts one LOVE runtime per Lua state. Activate this module's meter
	// before entering the otherwise-unmodified Box2D object implementation.
	void activate() const;
...
private:
	...
	// LOVE 11.5 uses one process-global meter. Dora keeps the configured value
	// on the state-local module and exposes it while that state's wrapper call
	// is active.
	float localMeter;
	static thread_local float meter;
}; // Physics
