...
Physics *activatePhysics(lua_State *L)
{
	Physics *physics = luax_getmodule<Physics>(L, Module::M_PHYSICS);
	if (physics != nullptr)
		physics->activate();
	return physics;
}

// Dora runs one independent Lua state per LoveNode. Resolve and activate the
// state-local module before calling LOVE's original Box2D implementation.
#define instance() activatePhysics(L)
...
int w_setMeter(lua_State *L)
{
	...
	luax_catchexcept(L, [&](){ instance()->setMeter(arg1); });
	return 0;

}
int w_getMeter(lua_State *L)
{
	lua_pushinteger(L, instance()->getMeter());
	return 1;
}
