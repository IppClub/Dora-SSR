...
int w_World_setGravity(lua_State *L)
{
	...
	luax_catchexcept(L, [&](){ t->setGravity(arg1, arg2); });
	return 0;
}
...
int w_World_getGravity(lua_State *L)
{
	...
	lua_remove(L, 1);
	int ret = 0;
	luax_catchexcept(L, [&](){ ret = t->getGravity(L); });
	return ret;
}
...
int w_World_setSleepingAllowed(lua_State *L)
{
	...
	luax_catchexcept(L, [&](){ t->setSleepingAllowed(b); });
	return 0;
}
...
int w_World_isSleepingAllowed(lua_State *L)
{
	World *t = luax_checkworld(L, 1);
	bool allowed = false;
	luax_catchexcept(L, [&](){ allowed = t->isSleepingAllowed(); });
	luax_pushboolean(L, allowed);
	return 1;
}
...
int w_World_isLocked(lua_State *L)
{
	World *t = luax_checkworld(L, 1);
	bool locked = false;
	luax_catchexcept(L, [&](){ locked = t->isLocked(); });
	luax_pushboolean(L, locked);
	return 1;
}
...
int w_World_getBodyCount(lua_State *L)
{
	World *t = luax_checkworld(L, 1);
	int count = 0;
	luax_catchexcept(L, [&](){ count = t->getBodyCount(); });
...
int w_World_getJointCount(lua_State *L)
{
	World *t = luax_checkworld(L, 1);
	int count = 0;
	luax_catchexcept(L, [&](){ count = t->getJointCount(); });
	lua_pushinteger(L, count);
	return 1;
}
...
int w_World_getContactCount(lua_State *L)
{
	World *t = luax_checkworld(L, 1);
	int count = 0;
	luax_catchexcept(L, [&](){ count = t->getContactCount(); });
	lua_pushinteger(L, count);
	return 1;
}
...
int w_World_isDestroyed(lua_State *L)
{
	...
	luax_pushboolean(L, w->isDestroyed());
	return 1;
}
