...
int w_Body_setAngularDamping(lua_State *L)
{
	Body *t = luax_checkbody(L, 1);
	float arg1 = (float)luaL_checknumber(L, 2);
	luaL_argcheck(L, std::isfinite(arg1) && arg1 >= 0.0f, 2,
		"damping must be finite and non-negative");
	luax_catchexcept(L, [&](){ t->setAngularDamping(arg1); });
	return 0;
}
...
int w_Body_setLinearDamping(lua_State *L)
{
	...
	luaL_argcheck(L, std::isfinite(arg1) && arg1 >= 0.0f, 2,
		"damping must be finite and non-negative");
	luax_catchexcept(L, [&](){ t->setLinearDamping(arg1); });
	return 0;
}
...
int w_Body_isDestroyed(lua_State *L)
{
	...
	luax_pushboolean(L, b->isDestroyed());
	return 1;
}
