...
template<>
KTM_INLINE void ktm::detail::comp_mul_implement::mul<float>(comp<float>& out, const comp<float>& x, const comp<float>& y) noexcept
{
    out.st = fc_mul_fc(x.st, y.st);
}

template<>
KTM_INLINE void ktm::detail::comp_mul_implement::act<float>(vec<2, float>& out, const comp<float>& c, const vec<2, float>& v) noexcept
{
    constexpr union { unsigned int i; float f; } mask { 0x80000000 };
