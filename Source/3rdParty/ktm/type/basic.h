...
template <typename T>
inline constexpr std::enable_if_t<std::is_floating_point_v<T>, T>
    euler = static_cast<T>(2.71828182845904523536028747135266249775724709369996);

template <typename T>
inline constexpr std::enable_if_t<std::is_floating_point_v<T>, T>
    pi = static_cast<T>(3.14159265358979323846264338327950288419716939937511);
...
template <typename T>
inline constexpr std::enable_if_t<std::is_floating_point_v<T>, T>
    sqrt_tow = static_cast<T>(1.41421356237309504880168872420969807856967187537695);
