...
b2DistanceProxy::b2DistanceProxy( const b2Shape* shape, int32 index )
{
	b2DistanceProxy::Set(shape, index);
}
...
void b2Distance( b2DistanceOutput* output, const b2DistanceInput* input )
{
	const b2DistanceProxy* proxyA = &input->proxyA;
	const b2DistanceProxy* proxyB = &input->proxyB;
	...
	b2Simplex simplex;
	simplex.Read(proxyA, transformA, proxyB, transformB);
	b2SimplexVertex* vertices = &simplex.m_v1;
	const int32 k_maxIters = 20;
	int32 saveA[3], saveB[3];
	int32 saveCount = 0;
