	...
	bool compileShader(const char* _varying, const char* _comment, char* _shader, uint32_t _shaderLen, const Options& _options, bx::WriterI* _shaderWriter, bx::WriterI* _messageWriter)
	{
		...
		else if ('c' == _options.shaderType) // Compute
		{
			...
			else
			{
				...
				bool preprocessOk = preprocessor.run(input);
				if (preprocessOk)
				{
			...
			else
			{
				...
				||  profile->lang == ShadingLang::ESSL)
				{
					...
					&&  profile->id >= 300)
					{
						...
						const bool hasBgfxFragColor = !bx::strFind(input, "bgfx_FragColor").isEmpty();
						if (hasFragColor || hasBgfxFragColor)
						{
							if (hasFragColor)
							{
				...
				if (preprocessOk)
				{
...

					{
						...
						||  profile->lang == ShadingLang::ESSL)
						{
							...
							// WebGL 2 already accepts the preprocessed GLSL ES 3.00
							// emitted here. Running it through the legacy glsl-optimizer
							// can hang on complex Love shaders (for example Balatro's
							// CRT vertex shader), so keep the Web path direct just like
							// the newer GLSL profiles.
							if ( (profile->lang == ShadingLang::GLSL && glsl_profile > 400)
							||   (profile->lang == ShadingLang::ESSL && glsl_profile > 300)
							||   (profile->lang == ShadingLang::ESSL
								&& 0 == bx::strCmpI(platform, "asm.js")) )
							{
								// Keep the complete ESSL header generated by shaderc. It contains
								// the WebGL version, attribute/varying declarations and fragment
								// output declaration. renderer_gl.cpp detects the #version header
								// and therefore does not inject a second compatibility header.
								code += preprocessor.m_preprocessed;
								if (profile->lang == ShadingLang::ESSL && _options.shaderType == 'f')
								{
									// Some Love effect sources reach the direct WebGL path after
									// `Image` has already expanded to `sampler2D`. In that form
									// the standard effect parameter named `texture` shadows the
									// GLSL texture() builtin and WebGL reports "function name
									// expected". Normalize this legacy Love parameter after
									// preprocessing, where its actual sampler type is known.
									auto replaceAll = [&code](std::string_view from, std::string_view to) {
										std::size_t offset = 0;
										while ((offset = code.find(from, offset)) != std::string::npos)
										{
											code.replace(offset, from.size(), to);
											offset += to.size();
										}
									};
								for (const std::string_view sampler : {
									std::string_view("sampler2D"), std::string_view("sampler2DArray"),
									std::string_view("samplerCube"), std::string_view("sampler3D")})
								{
									replaceAll(std::string(sampler) + " texture",
										std::string(sampler) + " loveTexture");
								}
								for (const std::string_view samplerFunction : {
									std::string_view("texture"), std::string_view("texture2D"),
									std::string_view("texture2DArray"), std::string_view("textureCube"),
									std::string_view("texture3D"), std::string_view("Texel")})
								{
											replaceAll(std::string(samplerFunction) + "(texture",
											std::string(samplerFunction) + "(loveTexture");
									}
								}
								bx::write(_shaderWriter, uint16_t(0), &err);
