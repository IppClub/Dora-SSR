...
_M.request = socket.protect(function(reqt, body)
    local requesturl = base.type(reqt) == "string" and reqt or reqt.url
    if requesturl and url.parse(requesturl, default).scheme == "https" then
        if _M.PROXY then return nil, "proxy not supported" end
        local https = require("ssl.https")
        https.TIMEOUT = _M.TIMEOUT
        return https.request(reqt, body)
    elseif base.type(reqt) == "string" then return srequest(reqt, body)
