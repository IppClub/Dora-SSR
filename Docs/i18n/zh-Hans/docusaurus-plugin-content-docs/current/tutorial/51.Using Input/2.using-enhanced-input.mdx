...
&emsp;&emsp;对于这种常见的“键盘或手柄二选一”场景，也可以使用更短的绑定写法：
...
-- 只监听动作完成状态
input:onCompleted("Ctrl+C", function(event)
	print("Ctrl+C 触发完成")
...
-- 只监听动作完成状态
input:onCompleted("Ctrl+C", function(event: InputEvent)
	print("Ctrl+C 触发完成")
	-- 移除当前生效的上下文，按下 Ctrl+C 便不再触发
	input:popContext()
end)
...
&emsp;&emsp;在这个示例中，我们创建了一个输入管理器，定义了一个输入上下文和一个动作。动作 `Ctrl+C` 触发器定义了按下键盘的 `Ctrl` 键和 `C` 键的触发条件。我们将这个上下文推入输入管理器进行激活。最后，我们通过 `onCompleted` 注册动作处理函数，当动作 `Ctrl+C` 完成时，打印一条消息并移除当前生效的上下文。

:::tip 提示
&emsp;&emsp;需要监听动作所有状态变化时，使用 `inputManager:on(actionName, handler)`；只关心动作完成时，使用 `inputManager:onCompleted(actionName, handler)`。处理函数会收到事件对象，其中包含 `state`、`progress` 和 `value`。当使用 Hold 或 Timed 这类和时间相关的触发器时，`progress` 的范围是 0 到 1。当使用摇杆轴这类会提供变化值的触发器时，可以读取 `value`。
:::
...
inputManager:on("Confirm", function(event)
	if event.state == "Ongoing" then
		print(string.format("确认中，进度：%d", event.progress * 100))
	elseif event.state == "Completed" then
		print("确认完成")
	end
end)

-- 只监听 Game 上下文中移动动作的完成状态
inputManager:onCompleted("MoveLeft", function(event)
	print("向左移动")
end)

inputManager:onCompleted("MoveRight", function(event)
	print("向右移动")
end)
```
...
-- 只监听 Game 上下文中移动动作的完成状态
inputManager:onCompleted("MoveLeft", function(event: InputEvent)
	print("向左移动")
end)

inputManager:onCompleted("MoveRight", function(event: InputEvent)
	print("向右移动")
end)
```
...
inputManager.on("Confirm", event => {
	if (event.state === TriggerState.Ongoing) {
		print(`确认中，进度：${event.progress * 100}`);
...
&emsp;&emsp;在这个示例中，我们创建了一个输入管理器，包含了两个上下文：`UI` 和 `Game`。`UI` 上下文包含了一个长按确认的动作 `Confirm`，`Game` 上下文包含了两个移动动作 `MoveLeft` 和 `MoveRight`。我们使用 `on` 处理确认动作的所有状态变化，使用 `onCompleted` 只处理移动动作的完成状态。
...
-- 用于定义一个同时支持键盘和游戏手柄按键的 QTE 挑战的输入上下文
local function QTEContext(keyName: Keyboard.KeyName, buttonName: Controller.ButtonName, timeWindow: number): {string: any}
	return {
		QTE = Trigger.Sequence({
			Trigger.Selector({
				-- 用于过滤特定键盘按键的触发器
...
&emsp;&emsp;接下来，我们需要处理输入事件，特别是 QTE 的逻辑。启动 QTE 只需要处理完成状态，可以使用 `onCompleted`；QTE 本身会有进行中、取消和完成等状态，所以使用 `on`。
...
-- 处理启动 QTE 挑战的输入事件
inputManager:onCompleted("StartQTE", function(event)
	phase = "Phase1"
	print("按下键盘 J 或按钮 A")
	inputManager:pushContext(phase)
	contextCount = contextCount + 1
...
inputManager:on("QTE", function(event)
	if event.state == "Ongoing" then
		...
		-- print(string.format("进度：%.2f", event.progress))
	elseif event.state == "Canceled" then
		if phase ~= "None" then
			phase = "None"
			inputManager:popContext(contextCount)
			print("挑战失败！")
...
-- 处理启动 QTE 挑战的输入事件
inputManager:onCompleted("StartQTE", function(event: InputEvent)
	phase = "Phase1"
	print("按下键盘 J 或按钮 A")
	inputManager:pushContext(phase)
	contextCount = contextCount + 1
...
inputManager.on("QTE", event => {
	if (event.state === TriggerState.Ongoing) {
		// 处理 QTE 的倒计时进度，progress 为从 0 递增到 1 的值
		// print(`进度：${event.progress.toFixed(2)}`);
