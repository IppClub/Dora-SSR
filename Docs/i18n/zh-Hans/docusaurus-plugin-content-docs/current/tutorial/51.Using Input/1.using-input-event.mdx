...
&emsp;&emsp;使用 `onMouseMove` 获取经过当前摄像机和节点变换自动转换后的鼠标位置，使用 `onMouseWheel` 监听鼠标滚轮事件。鼠标按键事件通常与触摸事件重叠，可以使用触摸事件的方法来处理鼠标点击。
...
- `onMouseMove`：鼠标移动时触发，无需按下鼠标按键。`touch.location` 是鼠标在节点局部空间中的位置，转换过程会自动处理当前摄像机的位置、旋转、缩放和投影，以及节点的 2D 或 3D 变换。如需当前视区逻辑坐标系中的位置，可使用 `touch.viewLocation`。
