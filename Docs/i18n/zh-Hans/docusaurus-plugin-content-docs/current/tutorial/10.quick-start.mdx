...
2. 第二步：编写游戏代码
	...
	- 如果您是在 Web IDE 之外开发凹语言工程，请先启动 Dora SSR 和 Web IDE，然后使用引擎内置 CLI 模式通过 `Dora cli wasm run wa Hello --host <web_ide_host>` 来构建、上传并运行项目。这里的 `Hello` 是 Dora SSR 资源树中已存在的目标目录名。如果需要，请将 `Dora` 替换为实际的 Dora 可执行文件路径。
	- 编写代码：
