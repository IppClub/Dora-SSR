...
&emsp;&emsp;TSX（TypeScript XML）是 TypeScript 的一种扩展，允许你在 TypeScript 代码中嵌入 JSX 标签，类似于在 JavaScript 中使用 JSX。这种结合为游戏开发带来了灵活且富有表达力的界面设计方式。使用 TSX，你可以以声明式方式构建游戏界面，类似于构建现代化的 Web 应用，同时利用 TypeScript 的类型检查。DoraX 既支持通过 `toNode()` 做一次性场景构建，也支持通过 `createRoot()` 配合 `signal()` 做动态 UI 更新。在[这里](Language%20Tutorial/using-tsx)，你可以开始学习如何在 Dora SSR 中使用 TSX。
...
&emsp;&emsp;Wa（凹语言）是一门简洁、可靠、静态类型的编程语言，专为 WebAssembly（WASM）环境设计。Dora SSR 内置 WASM 运行时，并提供 Wa 绑定，支持你直接用 Wa 作为脚本语言开发游戏逻辑和高性能模块。Wa 语法清晰，类型安全，适合追求高可靠性和跨平台性能的开发者。了解如何用编写 Wa 语言代码在 [这里](https://wa-lang.org/tutorial/)。
