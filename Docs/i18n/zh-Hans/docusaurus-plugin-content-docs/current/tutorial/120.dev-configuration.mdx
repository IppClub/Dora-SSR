...
1. 安装指定版本的 Web 工具链：[Emscripten](https://emscripten.org/docs/getting_started/downloads.html) `3.1.74`、[Rust](https://www.rust-lang.org/tools/install) `1.85.1`（含 `wasm32-unknown-emscripten` 目标）、[Go](https://go.dev/dl/) `1.24.3`、[Node.js](https://nodejs.org/) `22.14.0` 与 [CMake](https://cmake.org/download/) `3.30.5`。
...
2. 构建 Web 运行时与 Player。脚本会检查工具链，为 Emscripten 目标构建 Rust 运行时，把引擎编译为 WebAssembly，并在 `result/dora-web-player` 下打包出自包含的 Player。
	```sh
...
4. 构建前可通过 `DORA_WEB_PROFILE`（`core`、`dora-preset` 或 `custom`）以及诸如 `DORA_WEB_FEATURE_MODEL_3D=ON` 的功能开关调整运行时。构建目录默认为 `build/web`（`DORA_WEB_BUILD_DIR`），`DORA_WEB_JOBS` 控制构建并行度。
