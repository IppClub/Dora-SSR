...
2. 运行当前架构的编译脚本。它会在构建引擎前自动构建 SDL2、bgfx、Rust runtime 和 Wa 原生运行时。
...
flowchart TD
	A["选择构建入口"] --> B{"入口类型"}
	B -->|"run_macos.sh / run_windows.bat"| C["检查本地工具链"]
...
`run_macos.sh` 和 `run_windows.bat` 是本地桌面开发的最短路径：它们会检查必要工具，确认 SDL2、bgfx 和 Wa 原生产物是否存在；如果任一必要产物缺失，会先构建完整依赖，然后重新构建 Rust runtime 和平台工程，停止已有 Dora 进程，并使用仓库里的 `Assets` 目录启动 Debug 运行时。
