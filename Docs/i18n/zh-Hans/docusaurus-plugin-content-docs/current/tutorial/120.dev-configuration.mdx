...
`Tools/build-scripts` 下的平台依赖脚本会构建引擎需要的原生依赖，包括 SDL2、bgfx、Rust runtime 和 Wa 原生运行时。通常不需要再手动进入 `Source/3rdParty/bgfx` 单独构建 bgfx。

<Tabs groupId="platform-select">
<TabItem value="windows" label="Windows">

1. 安装 **Go 1.24 或更高版本**、**Rust**、**xmake**，以及带有 MSBuild 工具的 **Visual Studio Community 2026**，与 Windows GitHub Actions 构建环境保持一致。

2. 安装用于 Go cgo 构建 Wa 运行时 DLL 的 32 位 MinGW 工具链。CI 使用 MSYS2 MINGW32；请在 MSYS2 MINGW32 shell 中执行：
	```sh
...
4. 构建 Windows 运行时与解决方案。
	```bat
	Tools\build-scripts\build_windows.bat debug
	```

5. 如果需要本地调试，可在 Visual Studio 中打开 **Projects/Windows/Dora.sln**，运行 `Debug` 配置。

</TabItem>
<TabItem value="macos" label="macOS">

1. 安装 **Go 1.24 或更高版本**、**Rust**、**xmake** 和最新版 **Xcode**。
...
:::tip
Android 本地构建除了 **Android Studio** 外，还需要 **JDK 17**。Wa Android AAR 会通过 Go 和 gomobile 构建；如果本机缺少 gomobile，构建脚本会自动安装固定版本。
:::

1. 安装 **Go 1.24 或更高版本**、**Rust**、**xmake** 和最新版 **Android Studio**。
...
1. 安装 **Go 1.24 或更高版本**、**Rust** 和 **xmake**。
