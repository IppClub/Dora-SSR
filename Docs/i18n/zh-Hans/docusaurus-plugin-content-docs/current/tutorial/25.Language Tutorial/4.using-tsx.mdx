...
&emsp;&emsp;当动态根节点不再需要时，应当调用 `root.unmount()` 或 `root.dispose()`。这会移除它渲染出来的节点，取消它读取过的 signal 订阅，并避免后续 signal 变化继续更新这个 root。
...
- **`createRoot(parent)`**：在 Dora 节点下创建动态 TSX 根节点。调用 `root.render(enodeOrFunction)` 会将新的 TSX 树和上一次渲染结果做 diff，并更新子节点。`root.unmount()` 和 `root.dispose()` 会移除渲染出的节点并取消 signal 订阅。
