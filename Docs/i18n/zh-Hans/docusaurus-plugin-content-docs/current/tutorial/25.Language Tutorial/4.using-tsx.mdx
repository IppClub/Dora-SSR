...
&emsp;&emsp;`signal(value)` 用于创建一个轻量的响应式值。给 `signal.value` 赋值会在 Dora 的系统调度器上安排动态根节点更新。
...
- **`createRoot(parent)`**：在 Dora 节点下创建动态 TSX 根节点。调用 `root.render(enodeOrFunction)` 会将新的 TSX 树和上一次渲染结果做 diff，并更新子节点。
- **`signal(value)`**：创建响应式值。给 `signal.value` 赋值会调度动态根节点更新。
