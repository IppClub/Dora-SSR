...
local record ObjNode
	name: string
	children: {ObjNode}
end

-- 假设我们有一个场景对象列表，包含父子关系
local sceneObjects: {ObjNode} = {
	{
		name = "Root",
		children = {
			{
...
-- 递归函数，用于绘制场景树
local function drawSceneTree(nodes: {ObjNode})
	for _, node in ipairs(nodes) do
		if #node.children > 0 then
			ImGui.TreeNode(node.name, function()
				drawSceneTree(node.children)
...
// 递归函数，用于绘制场景树
function drawSceneTree(nodes: ObjNode[]) {
