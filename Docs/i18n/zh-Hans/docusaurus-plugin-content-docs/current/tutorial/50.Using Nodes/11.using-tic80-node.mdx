...
在文件开头添加 `-- tic80` 注释后，您将获得所有 TIC-80 API（如 `print`、`cls`、`rect`、`circ` 等）的完整自动补全支持，以及代码的类型检查。
...
`_G` 对象提供对 TIC-80 回调函数（如 `TIC`、`BOOT`、`BDR` 等）的访问。这允许您使用 TypeScript 开发 TIC-80 游戏，并获得完整的类型安全和编辑器支持。

:::info 运行时语言支持
Dora 引擎的 TIC80 运行时仅直接支持 **Lua** 和 **YueScript** 语言。其他语言（如 TypeScript 和 Teal）需要先编译为 Lua 代码，才能在 TIC-80 中执行。上述代码编辑支持有助于开发，但实际执行始终使用 Lua 或 YueScript。
:::
