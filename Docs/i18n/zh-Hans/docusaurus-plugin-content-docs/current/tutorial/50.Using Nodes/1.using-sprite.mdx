...
&emsp;&emsp;如果游戏中有很多小图，可以先把它们打包成 `.clip` 图集，再通过 `Sprite` 加载。完整流程请参考 [使用 TexturePacker 打包 Sprite 图片](./12.using-texture-packer.mdx)，其中包含从 `.clips` 目录到生成 `.png` 和 `.clip` 文件的步骤。
