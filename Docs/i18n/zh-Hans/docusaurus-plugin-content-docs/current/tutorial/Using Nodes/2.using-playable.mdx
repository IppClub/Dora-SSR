...
- **Model**：Dora SSR 引擎实现的骨骼动画系统。动画模型通常由一个 `.model` 文件，一个 `.clip` 文件和一个 `.png` 文件组成。
- **DragonBone**：开源的 DragonBones 动画系统。动画模型通常由一个以 `_ske.json` 结尾的文件，一个以 `_tex.json` 结尾的文件和一个以 `_tex.png` 结尾的图片文件组成。
- **Spine**：著名的商业动画软件 Spine2D 的动画系统。动画模型通常由一个 `.json` (或是 `.skel`) 文件，一个 `.atlas` 文件和一个 `.png` 文件组成。
...
由于 Spine2D 是商业软件，使用其动画需要遵守相应的许可协议，具体请参考 [Spine 官方网站](https://esotericsoftware.com/)。
:::
