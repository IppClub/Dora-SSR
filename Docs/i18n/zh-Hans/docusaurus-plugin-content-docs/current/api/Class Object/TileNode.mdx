...
| 参数名 | 类型 | 描述 |
| --- | --- | --- |
| tmxFile | string | 瓦片地图的 TMX 文件。 |
| layerName | string | TMX 文件中的地图层的名称。可以是使用 Tiled Map Editor (http://www.mapeditor.org) 创建的文件。进行加载的 TMX 文件必须是 XML 格式。 |
...
| --- | --- | --- |
| tmxFile | string | 瓦片地图的 TMX 文件。 |
| layerNames | \{string} | TMX 文件中的地图层的名称数组。可以是使用 Tiled Map Editor (http://www.mapeditor.org) 创建的文件。进行加载的 TMX 文件必须是 XML 格式。 |
