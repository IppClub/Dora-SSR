...
| 参数名 | 类型 | 描述 |
| --- | --- | --- |
| spineStr | string | 用于创建新实例的`Spine2D`文件名字符串。<br/>`Spine2D`文件名字符串可以是不带扩展名的文件路径，例如：“Spine/item”，也可以是带有所有相关文件的文件路径，例如 “Spine/item.skel|Spine/item.atlas” 或 “Spine/item.json|Spine/item.atlas”。 |
