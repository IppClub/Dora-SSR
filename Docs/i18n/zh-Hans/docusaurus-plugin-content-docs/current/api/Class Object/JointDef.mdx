...
| angularOffset | number | body-B 的角度减去 body-A 的角度，单位为度。 |
...
| lowerAngle | number | [可选] 下限角度限制，单位为度（默认为0.0）。 |
| upperAngle | number | [可选] 上限角度限制，单位为度（默认为0.0）。 |
| maxMotorTorque | number | [可选] 可施加到关节上以达到目标速度的最大扭矩（默认为0.0）。 |
| motorSpeed | number | [可选] 关节电机的目标速度，单位为度/秒（默认为0.0）。 |
...
| bodyB | Body | 第二个要连接到关节的物理体的名称。 |
| worldPos | Vec2 | 在世界中连接物理体的位置。 |
| axisAngle | number | 关节轴的角度，单位为度。 |
| maxMotorTorque | number | [可选] 关节电动机可以施加的最大扭矩，默认为 0.0。 |
| motorSpeed | number | [可选] 关节电动机的目标速度，单位为度/秒，默认为 0.0。 |
