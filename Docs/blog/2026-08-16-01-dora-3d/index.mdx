...
## Next, try it with your own model

At this point, you understand how to make the duck appear clearly by completing three steps: loading the model, positioning the camera, and adding light. The best next step is not to build a complex scene immediately. Replace `Duck.glb` with one of your own models and check the same three things again:

1. Does the model load successfully?
2. Can the camera see it from a suitable position?
3. After lighting is added, are the material and volume easy to read?

If the scene becomes empty again after you replace the model, do not change every parameter at once. Confirm that the asset loaded, adjust the camera, and then check the lighting. Different models use different scales and origins, so a change in composition is normal.

Once this smallest scene works, you can make the model move, add collision and character control, and place a game interface on top. The later tutorials also introduce measurement and debugging techniques, helping you decide whether a problem comes from the asset, camera, material, animation, or physics instead of relying on repeated parameter guessing.

The three-stage image in this article comes from an actual Dora SSR engine run. You can now open the complete tutorial, run the scene yourself, and replace the duck with your own model:

**[Start “Creating Your First 3D Scene”](/docs/tutorial/Three%20D%20Game%20Development/first-3d-scene/)**

---

<p align="center"><img src={require('./assets/sticker.webp').default} alt="3D topic sticker" height="320" /></p>
