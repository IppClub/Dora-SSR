...
Alternatively, install the unified Dora CLI and build, upload, and run the WASM file directly within the Rust project folder. The `Hello` parameter is the destination folder name in the Dora SSR resource tree, and the IP parameter should be the address shown in Dora SSR’s Web IDE after startup:
...
<p align="center">
  <img src={require('@site/static/img/article/rusty-3.jpg').default} alt="Using dora-cli for one-click compile, upload, and run"/>
   Using dora-cli for one-click compile, upload, and run
</p>
