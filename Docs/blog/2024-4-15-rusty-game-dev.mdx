...
Alternatively, start Dora SSR and its Web IDE, then use the built-in Dora CLI mode to build, upload, and run the WASM file directly within the Rust project folder. The `Hello` parameter is an existing destination folder name in the Dora SSR resource tree, and the IP parameter should be the address shown in Dora SSR's Web IDE after startup:

```shell
Dora cli wasm run rust Hello --host 192.168.3.1
```

<p align="center">
  <img src={require('@site/static/img/article/rusty-3.jpg').default} alt="Using built-in Dora CLI mode for one-click compile, upload, and run"/>
   Using built-in Dora CLI mode for one-click compile, upload, and run
</p>
