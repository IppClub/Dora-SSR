...
1. 建立一个独立的 **Dora 资源目录仓库**，只维护经过审核的资源元数据、版本提示、源地址、镜像地址和预览图。
...
4. 客户端安装资源源当前可取得的默认分支或指定 tag；首次安装完成后，仓库及其中的本地修改交由用户通过 Git 工具自行同步和维护。
5. 目录仓库是发现和审核入口，不是新的资源文件中转站。资源仓库失效时，应显式标记不可用或增加经过审核的 Git 镜像，而不是悄悄回退到旧 ZIP。

目录仓库本身可以直接 clone/pull；具体资源由下载器完成首次浅克隆并保留 `.git`，同时记录实际安装的 HEAD。下载器不再更新已安装工作树，后续 pull、分支、合并和本地修改都属于用户自己的 Git 工作流。
...
三个托管平台的代表性检查说明 AtomGit 和 GitCode URL 可以被 Git 直接访问，并会重定向到 `.git` 地址。唯一的 GitHub 资源地址在检查时已返回 `Repository not found`。这证明目录设计必须允许资源进入 `unavailable` 状态，也必须允许配置多个备用来源。
...
因此不需要引入外部 Git 可执行文件或新 Git 库。改造在下载器上方增加受限的资源安装层，统一执行来源选择、Git 树安全检查、临时目录安装和失败回滚；安装完成后不再接管仓库。
...
- 国内外网络环境都能取得可用的资源目录。
- 安装目录所列来源当前可取得的默认分支或指定 tag，并记录实际 HEAD。
...
- `ResourceGitInstaller`：只接受目录产生的结构化参数，负责首次 clone、资源树安全检查、进度、取消、临时目录安装和状态落盘。
...
- 项目 ID 全局唯一；同一资源的多个 Git 来源写在对应版本的 `sources` 中。
...
- `tag` 可选；配置后客户端浅克隆该 tag，否则浅克隆默认分支。
- `sources` 按角色和维护者建议排序；客户端按顺序使用第一个可成功安装的来源，不要求各来源 HEAD 完全一致。
...
当前资源都只有一个版本、仅 1 个有 tag。实际安装采用以下规则：

- tag 是可选的安装选择，但不是安全边界；未配置 tag 时安装默认分支当前 HEAD。
- 资源作者更新默认分支后，新用户可以直接安装新 HEAD，无需维护目录中的 commit 信息。
- 已安装状态保存实际 clone 得到的 HEAD，目录后续变化不会改写用户仓库。
...
- schema、字段类型、项目 ID、版本信息和来源 URL 能被当前客户端安全解析；
...
现有 `.dora/repo.json` 和 `.dora/banner.jpg` 在迁移期继续写入，避免其它入口失效。`resource-state.json` 记录下载器首次安装时的来源和实际 HEAD，不代表用户后续维护后的当前 HEAD，也不能作为下载器改写仓库的授权。

### 7.2 首次安装

1. 校验资源状态、目标目录和来源 URL。
2. 按来源顺序浅克隆到 `Content.writablePath/.download/<id>-<operation-id>`，不直接写最终目录。
3. 通过 clone 结果和 `verify-resource` 读取实际 HEAD，用于安全扫描和安装状态记录。
4. `verify-resource` 检查实际安装树，拒绝 submodule、`.gitmodules` 和任意 symlink。
5. 目录声明的入口仅用于生成运行元数据；入口暂时不存在不阻断仓库安装。
...
有 `tag` 时浅克隆该 tag；没有 tag 时浅克隆来源的默认分支。默认分支已经前进时直接安装新的 HEAD，并把实际值写入安装状态。目录 CI 应在发布前实际执行一次与客户端相同的浅克隆和资源树安全检查。
...
- UI 可以提示“目录中已有较新版本”，并提供仓库地址、打开 Git 面板或打开项目的入口，但不能代替用户更新。
...
- 每次切换来源都完成独立浅克隆和资源树安全检查。
- clone、LFS 展开或资源树安全检查失败时立即清理候选并回退。
- 成功安装后记录实际来源，供展示和故障诊断使用；下载器不自动改写已安装仓库的 remote。
- 目录 CI 定期检查所有 active 版本至少有一个可用来源。
- 上游失效时，由维护者定期检查并更新主目录仓库中的状态或来源信息。
...
- 目录 CI 和客户端都校验 `id`、相对路径、schema 和字段大小。
...
- UI 在卡片中显示来源、许可证状态和版本名；实际安装 HEAD 记录在本地状态中。首次运行第三方代码的二次提示可作为后续安全增强。
...
- 版本下拉框显示版本名。
- 详情 tooltip 展示 upstream、当前传输来源、许可证状态和兼容性。
...
3. CI 对每个来源执行 `ls-remote`，确认仓库可访问。
4. CI 用客户端等价参数完成浅克隆，并验证 LFS、submodule 和 symlink 策略。
...
- 所有版本均保留名称和发布时间；暂未明确许可证的资源标为 `pending`。
- 已删除确认失效的项目。
...
- 实现临时 clone、实际资源树安全检查、LFS、取消和 `.dora/resource-state.json`。
...
- submodule、内部 symlink、越界 symlink，以及入口缺失但仍允许安装。
- 安装后 `UpdateEntries`、默认入口、多入口、不可运行资源和删除流程。

### 12.3 验收标准

- 正式安装完成时自动记录实际 HEAD，目录不维护资源 commit。
...
4. 维护者定期检查失效上游，并通过更新主目录仓库中的资源状态或源地址进行维护。
5. 目录来源不绑定组织名、固定地址或签名公钥；资源安装只阻断于 clone、LFS、资源树安全检查和原子落盘，不阻断于入口文件存在性。
...
- active 资源的浅克隆、实际资源树安全检查、LFS、取消与临时目录安装。
...
- `ResourceGitInstaller.ts`：首次浅克隆、实际 HEAD 记录、Git 树安全检查、来源回退、元数据和原子安装；入口缺失不阻断安装。
- `ResourceDownloader.ts`：目录状态、筛选、分页、按页异步预览、最多 36 个不同图片的 LRU、安装/运行/删除交互。
- Wa `gitjobs`：资源路径只保留 `verify-resource`，检查实际安装 commit 的树结构；catalog 不再有签名验证命令或内置 key。
...
- 安装：从 AtomGit 通过内置 Git 安装 `GrowlR-NNG`，自动记录实际 HEAD，保留 `.git` 并写入两份 `.dora` 元数据，随后从下载器成功启动该 Mini Game。
