...
- Cursor 是隔离 state 中的真实 userdata，而不是 Dora 全局 Cursor 的状态回显。自定义 Cursor 从 ImageData 或经 Dora Content 解码的 filename/FileData 复制 RGBA8 像素与 hotspot；十二种 system cursor 按 Runtime 缓存，重复查询保持 userdata identity。`setCursor` 通过 registry 强引用当前对象，`getCursor` 返回同一对象；替换、默认恢复、GC、restart 和 state close 都必须按所有权释放 SDL handle。宿主不支持 cursor 时 `isCursorSupported` 返回 false，创建失败保留 SDL 定向错误。
...
`World:setCallbacks(begin, end, pre, post)` 直接安装到该 LoveNode 私有 PhysicsWorld 的 PlayRho contact listener，不经会丢失 ShapeID 的 Dora Body 事件层。listener 把 ShapeID 还原为原 Fixture userdata，并在 begin 到 end 期间为同一 native contact 保持一个由 Lua registry 强引用的 Contact userdata；end callback 返回后立即从 registry 和 backend 映射中移除，之后 `isValid()` 为 false，其余方法报明确的 invalid Contact 错误。当前 Contact 子集包含 fixtures、转换为 Lua 1-based 的 shape child indices、positions、normal、friction/restitution 的 get/set/reset、enabled、touching 和 tangent speed；preSolve 的修改直接作用于当前 PlayRho contact，postSolve 依 Love 参数顺序传递法向/切向 impulse 对。
...
- Love 11.5 tag 本身不含 `testing/`；官方仓库后续测试树明确以开发中的 Love 12 为目标。接入时固定官方测试 commit，并只选择在 Love 11.5 仍存在且语义一致的方法；Love 12-only API 必须以版本原因跳过，不能计为 Dora 失败或通过。
