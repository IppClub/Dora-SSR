...
| 条件 | 处理模式 | 额外行为 |
| --- | --- | --- |
| 元素 `type` 变化 | 子树重建 | 旧节点递归 unmount，新节点重新 mount |
| `key` 变化 | 子树重建 | 同上 |
| 新元素是 `<draw-node>` | 宿主重建 | draw shapes 是即时绘制命令，目前不做 shape diff |
| `onMount` 新旧值变化、增加或删除 | 宿主重建 | `onMount` 只在 mount 时调用；已挂载子节点会迁移到新宿主节点后继续 diff |
| 非 patchable 的 `on*` 属性变化、增加或删除 | 宿主重建 | 当前剩余典型项是 `custom-node.onCreate`；详见下表 |
| 元素命中特定结构属性变化规则 | 宿主重建 | 见“元素级重建规则”；已挂载子节点会迁移到新宿主节点后继续 diff |
