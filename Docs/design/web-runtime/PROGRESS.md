...
| P0-04 | 建立 `web-player-minimal` feature profile | P0-01 | 已完成 | Codex | 只包含 Lua、最小渲染、Content 和启动资源 | 独立 `LuaBindingWeb.pkg`/`WebInitialization.lua` 只暴露 Application、Content、Scheduler、Node、Director.ui、DrawNode、Sprite、Label 与日志；Player 排除完整 binding/manual、Yue/Teal、Wa/Wasm、Love、Platformer、Entity、ML、2D/3D 物理、3D 节点和 Rust bridge；无 Asyncify 的 release 包在 Chrome 152 完成 manifest、Lua 首帧和 lazy asset 验证 |
...
| R-03 | 架构 | Async 全部同步内联会改变回调时序并冻结页面 | 初版平台队列已实现并由浏览器探针验证 | `Source/Platform/Web/WebTaskQueue` 通过逐任务 event-loop callback 延迟执行，Async 支持取消代次；P2/P3 仍需为 Fetch/解码接 Promise/Worker，CPU 重任务不能停留在主线程 |
