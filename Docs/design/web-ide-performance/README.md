...
- App 不再为全部 `files` 构造完整活动视图。
- Monaco Model 独立于 React Editor 实例保留。
- 普通编辑器只挂载当前 Tab；必要时保留最近 1–2 个重型可视化编辑器作为 LRU。
- 关闭 Tab 时统一释放 Editor、Observer、Worker 关联和大型预览资源。

当前实现采用“活动 Tab + 最近 1 个 Tab”的两实例预算。普通文本 Tab 离开后释放
Monaco Editor、保留 Model 和 view state；Yarn 在变更后异步保存 JSON→文本快照，
快照完成前固定 iframe，CodeWire 在变更和卸载时同步保存 visual script。两者超出预算
后均可卸载并从未保存快照恢复。TIC 当前桥接只提供读盘和写盘，缺少“不落盘导出内存
cart”的接口，因此采用 dirty-pin：尚未发生键盘/指针操作的干净实例可被 LRU 回收，
可能含未保存状态时固定挂载；保存成功且保存期间没有新输入后解除固定。这样不会自动写入
用户文件，也不会因回收丢失内存 cart。TIC 的顶层 `message` 和 iframe 输入监听已具备
重载/卸载清理。顶部项目 Tab 隐藏时会继续
保留最近视图的快速返回能力，但其 Agent 子面板收到 `active=false`，不再执行轮询和滚动工作。
