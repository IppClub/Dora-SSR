...
| --- | --- | --- |
| handler | function | The function to be called.<br/>It should take a single argument of type number, which represents the delta time since the last frame.<br/>If the function returns true, it will not be called again. |
...
&emsp;&emsp;Manually updates the scheduler if it is created by the user.
...
| --- | --- |
| boolean | Returns true if the scheduler has stopped. |