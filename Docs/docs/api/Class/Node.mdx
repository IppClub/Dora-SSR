...
&emsp;&emsp;Clears all functions scheduled with `onUpdate`.

**Signature:**
```tl
clearUpdate: function(self: Node)
```
...
&emsp;&emsp;Clears all functions registered with `onRender`.

**Signature:**
```tl
clearRender: function(self: Node)
```
...
&emsp;&emsp;Registers a callback for mouse movement, even when no mouse button is pressed.
...
**Signature:**
```tl
onMouseMove: function(self: Node, callback: function(touch: Touch))
```
...
**Signature:**
```tl
onTextEditing: function(self: Node, callback: function(text: string, startPos: integer, length: integer))
```
...
**Signature:**
```tl
onAppWS: function(self: Node, callback: function(event: Dictionary))
```
