...
&emsp;&emsp;Gets the Model3D or environment load state.

**Signature:**
```tl
getLoadState: function(self: Cache, filename: string): LoadState
```
...
&emsp;&emsp;Gets the latest Model3D or environment load error.

**Signature:**
```tl
getLoadError: function(self: Cache, filename: string): string
```
...
&emsp;&emsp;Cancels an active Model3D or environment load.

**Signature:**
```tl
cancelLoad: function(self: Cache, filename: string): boolean
```
...
&emsp;&emsp;Unloads all resources from the cache.
