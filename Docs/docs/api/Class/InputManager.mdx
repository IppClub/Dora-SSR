...
local Trigger = InputManager.Trigger
...
inputManager:onCompleted("action1", function(event)
...
&emsp;&emsp;Gets the global event name for an action.

**Signature:**
```tl
getEventName: function(self: InputManager, actionName: string): string
```
...
&emsp;&emsp;Listens for all state changes of an action.

**Signature:**
```tl
on: function(self: InputManager, actionName: string, handler: InputHandler)
```
...
| --- | --- | --- |
| actionName | string | The action name. |
| handler | InputHandler | The handler to call. |
...
&emsp;&emsp;Listens for the next state change of an action, then disables the listener.

**Signature:**
```tl
once: function(self: InputManager, actionName: string, handler: InputHandler)
```
...
&emsp;&emsp;Disables handlers registered by `on()`, `once()`, or `onCompleted()`.

**Signature:**
```tl
off: function(self: InputManager, actionName: string, handler: InputHandler)
```
...
| --- | --- | --- |
| actionName | string | The action name. |
| handler | InputHandler | The handler to disable. |
...
&emsp;&emsp;Listens for the completed state of an action.

**Signature:**
```tl
onCompleted: function(self: InputManager, actionName: string, handler: InputHandler)
```
