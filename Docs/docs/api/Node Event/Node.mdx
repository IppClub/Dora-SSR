...
&emsp;&emsp;The MouseMove slot is triggered when the mouse moves, even when no mouse button is pressed.
...
**Signature:**
```tl
["MouseMove"]: function(touch: Touch)
```
...
**Signature:**
```tl
["TextEditing"]: function(text: string, startPos: integer, length: integer)
```
