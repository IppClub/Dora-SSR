...
&emsp;&emsp;Triggers when a websocket connection gets an event.
The callback receives one object argument:
	`event.type` could be "Open", "Close", "Send", "Receive".
	`event.msg` contains the message payload.
