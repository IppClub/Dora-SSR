...
```sh
Dora cli doc search Sprite -l en --source tutorial --lang ts
...
```sh
Dora cli doc read ts/using-sprite.md -l en --source tutorial --start 1 --end 80
```

`--source` can be `api` or `tutorial`. `--lang` selects the API or tutorial language variant. `teal` and `tl` both use the Teal docs.
