...
## 3. TypeScript and Script Projects
...
Build the project. When the language is not specified, Dora CLI scans supported source files and builds each one by extension. Use `--lang` only when you want to build one language family.
...
Supported build languages are `all`, `ts`, `yue`, `tl`, `xml`, and `wa`. `all` is the default.
