...
-- Or you can mount it to the root node of the game scene explicitly
...
1. **Registering an Event**: You can register an event on a node to listen to it. And the event is triggered by the built-in engine features, such as screen touch, mouse, keyboard, and gamepad inputs. You can use the following code pattern to register and listen to events on a node. The event triggering on the node is multicast, meaning you can register multiple callback functions to listen to the same event. Here's an example code:
...
3. **Removing an Event**: When you no longer need to listen to an event, you can remove the event listener using the following methods. Here's an example code:
...
-- Or you can remove all event listeners
...
To simplify event handling, we also provide some built-in event registering functions, such as `onTapBegan`, `onTapEnded`, `onTapped`, `onTapMoved`, `onMouseWheel`, etc. You can register and handle events by calling these functions on the node.

For example, you can register and handle the tap began event by calling the following code:
