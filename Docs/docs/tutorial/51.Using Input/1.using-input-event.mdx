...
Use `onMouseMove` to receive pointer positions automatically converted through the current camera and the node's transform, and use `onMouseWheel` to listen for mouse wheel events. Mouse button events generally overlap with touch events, and mouse clicks can be handled using touch event methods.
...
local function getMousePositionInNode(target: Node.Type): Vec2.Type
