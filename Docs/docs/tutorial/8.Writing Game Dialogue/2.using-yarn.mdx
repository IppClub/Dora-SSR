...
## 5. Adding Custom Commands and States

&emsp;&emsp;You can add custom commands and initial state variables by passing the `command` and `state` parameters to `YarnRunner`. The `command` parameter is a Lua table containing callback functions that can execute commands, and the `state` parameter is a table containing predefined variables. Here is an example:
...
&emsp;&emsp;Then, in the Yarn script, you can change the initial variable values by using the `<<set $variableName = expression>>` command and print the variable values using the custom command `<<print $variableName>>`:
