...
:::tip Teal for Dora SSR
This tutorial is a slightly modified version of the original Teal tutorial, adapted for Dora SSR. The original tutorial can be found [here](https://github.com/teal-language/tl/blob/master/docs/tutorial.md). And this tutorial assumes you are already familiar with Lua, you can refer to the [Lua official documentation](https://lua-users.org/wiki/TutorialDirectory) for learning.
:::
...
string. For an example of a custom user-written iterator, see the [Functions](#13-functions)
section below.
...
[union](#14-union-types) of all the types in the tuple (following the
...
### 14.1 Current limitations of union types
...
### 16.2 To-be-closed variables

The `<close>` annotation from Lua 5.4 is only supported in Teal if your
code generation target is Lua 5.4. These work just
