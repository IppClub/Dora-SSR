...
TSX (TypeScript XML) extends TypeScript, allowing JSX tags in TypeScript code, similar to JSX in JavaScript. This combination brings flexibility and expressiveness to game UI design. With TSX, you can build game interfaces declaratively, like modern web apps, benefiting from TypeScript's robust type checking. DoraX supports both one-shot scene construction with `toNode()` and dynamic UI updates with `createRoot()` plus `signal()`. Start using TSX [here](Language%20Tutorial/using-tsx).
