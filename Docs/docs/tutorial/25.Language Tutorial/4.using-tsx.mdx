...
Call `root.unmount()` or `root.dispose()` when a dynamic root is no longer needed. This removes its rendered nodes, unsubscribes it from any signals it read, and prevents later signal changes from updating that root.
...
- **`createRoot(parent)`**: Creates a dynamic TSX root under a Dora node. Calling `root.render(enodeOrFunction)` updates children by diffing new TSX trees against previous ones. `root.unmount()` and `root.dispose()` remove rendered nodes and unsubscribe the root from signals.
