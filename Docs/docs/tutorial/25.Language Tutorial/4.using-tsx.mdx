...
`signal(value)` creates a small reactive value. Assigning to `signal.value` schedules dynamic roots to update on Dora's system scheduler.
...
- **`createRoot(parent)`**: Creates a dynamic TSX root under a Dora node. Calling `root.render(enodeOrFunction)` updates children by diffing new TSX trees against previous ones.
- **`signal(value)`**: Creates a reactive value. Assigning `signal.value` schedules dynamic roots to update.
