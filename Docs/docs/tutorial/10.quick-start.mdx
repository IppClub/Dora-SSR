...
2. Step Two: Write the game code
	...
	- If you are developing a Wa project outside the Web IDE, first start Dora SSR and its Web IDE, then use the built-in Dora CLI mode to build, upload, and run it with `Dora cli wasm run wa Hello --host <web_ide_host>`. Here `Hello` is an existing destination folder name in the Dora SSR resource tree. Replace `Dora` with the path to your Dora executable if needed.
	- Write the code:
