...
2. Step Two: Write the game code
	...
	- If you are developing a Wa project outside the Web IDE, you can also use [dora-cli](https://github.com/IppClub/Dora-SSR/tree/main/Tools/dora-cli) to build, upload, and run it with `dora wa run Hello --host <web_ide_host>`. Here `Hello` is the destination folder name in the Dora SSR resource tree.
	- Write the code:
