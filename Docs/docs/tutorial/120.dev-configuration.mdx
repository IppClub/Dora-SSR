...
The platform library scripts under `Tools/build-scripts` build the native dependencies required by the engine, including SDL2, bgfx, the Rust runtime, and the Wa native runtime. You normally do not need to enter `Source/3rdParty/bgfx` and build bgfx manually.
...
2. Install a 32-bit MinGW toolchain for Go cgo when building the Wa runtime DLL. The CI build uses MSYS2 MINGW32; run this from an MSYS2 MINGW32 shell:
	```sh
...
3. Build native dependencies.
	```bat
	Tools\build-scripts\build_lib_windows.bat debug
	```

4. Build the Windows runtime and solution.
	```bat
	Tools\build-scripts\build_windows.bat debug
	```

5. For local debugging, open **Projects/Windows/Dora.sln** in Visual Studio and run the `Debug` configuration.

</TabItem>
<TabItem value="macos" label="macOS">

1. Install **Go 1.24 or later**, **Rust**, **xmake**, and the latest **Xcode**.

2. Build native dependencies.
	```sh
	Tools/build-scripts/build_lib_macos.sh debug
	```
...
:::tip
Local Android builds require **JDK 17** in addition to **Android Studio**. The Wa Android AAR is built with Go and gomobile; the build script installs the pinned gomobile version automatically if it is missing.
:::

1. Install **Go 1.24 or later**, **Rust**, **xmake**, and the latest **Android Studio**.
...
1. Install **Go 1.24 or later**, **Rust**, and **xmake**.
