...
1. Install the pinned Web toolchain: [Emscripten](https://emscripten.org/docs/getting_started/downloads.html) `3.1.74`, [Rust](https://www.rust-lang.org/tools/install) `1.85.1` with the `wasm32-unknown-emscripten` target, [Go](https://go.dev/dl/) `1.24.3`, [Node.js](https://nodejs.org/) `22.14.0`, and [CMake](https://cmake.org/download/) `3.30.5`.
...
2. Build the Web runtime and Player. The script checks the toolchain, builds the Rust runtime for the Emscripten target, compiles the engine to WebAssembly, and packages a self-contained Player into `result/dora-web-player`.
	```sh
...
4. To tune the runtime before building, set `DORA_WEB_PROFILE` (`core`, `dora-preset`, or `custom`) and feature switches such as `DORA_WEB_FEATURE_MODEL_3D=ON`. The build directory defaults to `build/web` (`DORA_WEB_BUILD_DIR`), and `DORA_WEB_JOBS` controls build parallelism.
