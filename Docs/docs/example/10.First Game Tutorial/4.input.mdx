...
Bind player input events to control the player's movement. Assume we have a `player` entity and bind the controls for up, down, left, and right movement. For new code, use `inputManager.onCompleted()` so the callback receives a structured input event and you do not need to build the `Input.` event name manually.
...
inputManager.onCompleted('Up', () => y = 1);
inputManager.onCompleted('Down', () => y = -1);
inputManager.onCompleted('Left', () => x = -1);
inputManager.onCompleted('Right', () => x = 1);
