...
export default function Play() {
	...
	const base = siteConfig.customFields.galleryBaseUrl as string;
	...
	return <Layout title={zh ? '在线试玩' : 'Play'}>
		<main className={`container ${styles.gallery}`}>
			...
			{active && <section className={styles.stage}>
				<div className={styles.toolbar}><h2>{active.title}</h2>
					<button onClick={() => setGeneration(value => value+1)}>{zh ? '重新开始' : 'Restart'}</button>
					<button onClick={() => frame.current?.requestFullscreen().catch(() => setError(zh ? '请使用独立窗口打开游戏。' : 'Please open the game in a new window.'))}>{zh ? '全屏' : 'Fullscreen'}</button>
					<a href={url} target="_blank" rel="noopener noreferrer">{zh ? '独立窗口' : 'New window'}</a>
					<button onClick={() => setActive(null)}>{zh ? '关闭游戏' : 'Close'}</button>
				</div>
