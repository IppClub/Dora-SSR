...
export class TransformationContext {
    ...
    constructor(public program: ts.Program, public sourceFile: ts.SourceFile, private visitorMap: VisitorMap) {
        // Explicit initialization preserves order under both TS class-field modes.
        // Consumers no longer inherit the old IDE's tsconfig after packaging.
        this.checker = program.getTypeChecker() as TypeCheckerWithEmitResolver;
        this.options = program.getCompilerOptions();
        this.luaTarget = this.options.luaTarget ?? LuaTarget.Universal;
        this.isModule = ts.isExternalModule(sourceFile);
        this.isStrict = (this.options.alwaysStrict ?? this.options.strict) ||
