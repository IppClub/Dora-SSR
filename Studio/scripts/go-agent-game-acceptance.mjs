...
try{
  ...
  await allowances.getByLabel('授权账号名').fill('go-agent-creator');
  await allowances.getByRole('button',{name:'读取账号授权'}).click();
  await allowances.getByText('尚未分配账号模型额度').waitFor();
  await allowances.getByLabel('账号金额上限').fill('10');
  await allowances.getByLabel('账号模型并发').fill('2');
  await allowances.getByRole('button',{name:'保存账号总额度'}).click();
  await allowances.getByText('已用 ¥0',{exact:false}).waitFor();
  await allowances.getByLabel('选择授权 API').selectOption({label:'Go Agent 验收模型 · 可用'});
  await allowances.getByLabel('逐 API 金额上限').fill('5');
  await allowances.getByLabel('逐 API 并发').fill('1');
  await allowances.getByRole('button',{name:'分配逐 API 授权'}).click();
  await allowances.getByText('已有授权',{exact:false}).waitFor();
