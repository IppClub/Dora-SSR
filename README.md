...
* **Moonscript语言**  
&emsp;&emsp;Moonscript对于Lua，等同于Coffeescript对于Javascript。是Lua的一种方言，并且编译为Lua语言由Lua解释器加载执行。同时它的语法和Coffeescript的语法非常接近，写出的代码表达力强又极为简洁。非常适合于快速编写复杂的业务逻辑。编写一行Moonscript做的工作可能相当于编写多行Lua代码才能做到同样的工作，所以在SSR项目中极为推荐，并进行了支持作为默认的应用开发语言。使用时需要替换Lua的debug.traceback()函数来显示错误信息。或是将写好的Moonscript代码文件编译为Lua代码文件，再进行执行调试。  
