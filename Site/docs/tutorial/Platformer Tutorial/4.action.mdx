...
&emsp;&emsp;Welcome to the fourth tutorial of the Dorothy SSR game engine side-scrolling 2D game development series! In this tutorial, we will learn how to write the game character action module. In a game, the actions of the characters are a crucial part as they determine the behavior and reactions of the characters. We will use the [UnitAction](../../api/Class%20Object/Platformer/UnitAction) class from the Dorothy SSR game engine to define and manage the character actions.
...
&emsp;&emsp;Next, we use the [add](../../api/Class%20Object/Platformer/UnitAction#add) method of UnitAction to add new actions. Each action has a name and a set of [parameters](../../api/Class/Platformer/UnitActionParam) that define its behavior. For example, we can define an "idle" action that represents the character's behavior in an idle state:
