...
&emsp;&emsp;Then, we create a [PlatformWorld](../../api/Class/Platformer/PlatformWorld) object and set the camera's boundary, follow ratio, and zoom. Here, [world.camera](../../api/Class/Platformer/PlatformWorld#camera) is a [PlatformCamera](../../api/Class/Platformer/PlatformCamera) object, which is a camera for 2D platform games that can track the movement of game units and keep them within the camera's view. [boundary](../../api/Class/Platformer/PlatformCamera#boundary) is the rectangular region of the camera's visible area, [followRatio](../../api/Class/Platformer/PlatformCamera#followratio) is the rate at which the camera moves to follow the target position, and [zoom](../../api/Class/Platformer/PlatformCamera#zoom) is the scaling factor of the camera. A value of 1.0 represents normal size, and 2.0 represents a zoomed-in view that is twice the normal size.
...
&emsp;&emsp;Next, we create a [BodyDef](../../api/Class/BodyDef) object, which is used to describe the definition of physical objects in the game scene. We attach four polygons that will serve as the collision boundaries for the game scene:
...
&emsp;&emsp;Then, we create a [Body](../../api/Class/Body) object, which is a physical entity created from the terrainDef object and also a type of scene node that can be attached to the game scene. We set its order and group, and then add four Rectangle objects as visual objects for the scene. These Rectangle objects are graphic nodes used to display the physical scene:
...
&emsp;&emsp;Next, we add the terrain object to the world object and then add the world object to [Director.entry](../../api/Class/Director#entry). Here, Director.entry is the interface provided by the engine to create the root node of the game scene tree:
...
&emsp;&emsp;Then, we save the world object in [Platformer.Data.store](../../api/Class/Platformer/Data#store) so that other modules can access it:

```tl title=Script/Scene.tl
Platformer.Data.store["Scene:world"] = world
```

&emsp;&emsp;Finally, we create an [Entity](../../api/Class/Entity) object and call the `Loader.loadExcel()` function. The specific purposes of these two lines of code will be explained in subsequent tutorials:
