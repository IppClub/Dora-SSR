...
Then, we create a root node for the UI called `alignNode` and add it to [Director.ui](../../api/Class/Director#ui). This node serves as the parent node for all other UI nodes.
