...
Initially, Yuescript was tied to an open-source game engine project, Dora SSR (https://dora-ssr.net/zh-Hans), aiming to elevate the capabilities of the Lua-supported engine with an advanced version of Moonscript. Along with the engine’s Web IDE, Yuescript also adds modest code completion and type inference features to enhance coding efficiency.
