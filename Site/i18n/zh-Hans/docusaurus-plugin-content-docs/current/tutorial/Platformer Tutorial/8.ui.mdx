...
&emsp;&emsp;在这篇教程中，我们将介绍如何使用Dorothy SSR游戏引擎创建游戏的UI模块。我们将使用两种方式创建UI：一种是使用基于游戏场景节点编写的UI功能组件，一种是使用ImGui框架的接口。然而，需要注意的是，ImGui在实际开发中并不推荐直接用于创建游戏UI，而是建议主要用于开发一些游戏辅助的调试UI。
...
&emsp;&emsp;然后，我们创建了一个UI的根节点`alignNode`，并将其添加到[Director.ui](../../api/Class/Director#ui)中。这个节点是所有其他UI节点的父节点。

```tl title=Script/UI.tl
-- 获取设备的DPI缩放值使UI在各种设备上的显示大小一致
...
&emsp;&emsp;接着，我们创建了一个左对齐的节点`leftAlign`，并将其添加到`alignNode`中。然后，我们在`leftAlign`中创建了一个菜单`leftMenu`。
...
&emsp;&emsp;在`leftMenu`中，我们创建了三个圆形按钮：`leftButton`、`rightButton`和`jumpButton`。这三个按钮分别用于控制玩家的左移、右移和跳跃操作。每个按钮都有一个[TapBegan](../../api/Node%20Event/Node#tapbegan)事件和一个[TapEnded](../../api/Node%20Event/Node#tapended)事件，分别在按钮被按下和按钮被释放时触发。
