...
&emsp;&emsp;接着，我们使用[决策树框架](../../api/Class/Platformer/Decision)来创建AI。决策树是一种常用的AI设计模式，它可以根据一系列条件来决定执行哪个动作。在这个例子中，我们使用了[Sel](../../api/Class/Platformer/Decision#sel)（选择器节点）和[Seq](../../api/Class/Platformer/Decision#seq)（序列节点）来组织决策树，[Con](../../api/Class/Platformer/Decision#con)（条件节点）来检查条件，[Act](../../api/Class/Platformer/Decision#act)（动作节点）来执行动作。
