...
| --- | --- | --- |
| child | Node | 要添加的子节点。 |
| order | integer | 子节点的绘制顺序。默认为0。 |
| tag | string | 子节点的标签。默认为空字符串。 |
...
| order | integer | 当前节点的绘制顺序。默认为0。 |
| tag | string | 当前节点的标签。默认为空字符串。 |
...
| --- | --- | --- |
| size | Size | 用于对齐排布的范围。 |
| padding | number | 子节点之间的间距（默认值为10）。 |
...
| 参数名 | 类型 | 描述 |
| --- | --- | --- |
| enabled | boolean | 是否启用或禁用抓取器。默认为 true。 |
