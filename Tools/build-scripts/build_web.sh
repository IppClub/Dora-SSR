...
MUSIC_FEATURE="${DORA_WEB_FEATURE_MUSIC:-AUTO}"
...
RUST_TARGET_DIR="$ROOT_DIR/Source/Rust/target"
if [[ "$BUILD_PTHREADS" == "1" ]]; then
	# A pthread Player needs Rust's standard library rebuilt with wasm atomics.
	# Keep it separate so a later non-threaded build cannot reuse the wrong archive.
	RUST_TARGET_DIR="$ROOT_DIR/Source/Rust/target-web-pthreads"
fi
RUST_RUNTIME="$RUST_TARGET_DIR/$DORA_WEB_RUST_TARGET/release/libdora_runtime.a"
...
if [[ "$MUSIC_FEATURE_UPPER" == "ON" || "$MODEL_3D_FEATURE_UPPER" == "ON" ]]; then
	...
	if [[ "$BUILD_PTHREADS" == "1" ]]; then
		RUST_BUILD_ARGS=(-Z build-std=std,panic_abort "${RUST_BUILD_ARGS[@]}")
		RUST_BUILD_FLAGS="$RUST_BUILD_FLAGS -C target-feature=+atomics,+bulk-memory,+mutable-globals"
		RUSTC_BOOTSTRAP=1 RUSTC_WRAPPER='' CARGO_TARGET_DIR="$RUST_TARGET_DIR" RUSTFLAGS="$RUST_BUILD_FLAGS" cargo build "${RUST_BUILD_ARGS[@]}"
	else
		RUSTC_WRAPPER='' CARGO_TARGET_DIR="$RUST_TARGET_DIR" RUSTFLAGS="$RUST_BUILD_FLAGS" cargo build "${RUST_BUILD_ARGS[@]}"
	fi
...
for feature in PHYSICS_2D ENTITY PLATFORMER BUILTIN_LIBS ML YUE LOVE MODEL_3D MUSIC; do
