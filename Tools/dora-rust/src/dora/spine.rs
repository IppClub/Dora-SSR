...
impl Spine {
	...
	pub fn contains_point(&mut self, x: f32, y: f32) -> String {
		return crate::dora::to_string(unsafe { spine_contains_point(self.raw(), x, y) });
	}
	pub fn intersects_segment(&mut self, x_1: f32, y_1: f32, x_2: f32, y_2: f32) -> String {
		return crate::dora::to_string(unsafe { spine_intersects_segment(self.raw(), x_1, y_1, x_2, y_2) });
	}
	pub fn with_files(skel_file: &str, atlas_file: &str) -> Spine {
		return Spine { raw: unsafe { spine_with_files(crate::dora::from_string(skel_file), crate::dora::from_string(atlas_file)) } };
	}
	pub fn new(spine_str: &str) -> Spine {
		return Spine { raw: unsafe { spine_new(crate::dora::from_string(spine_str)) } };
	}
