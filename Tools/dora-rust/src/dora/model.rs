...
impl Model {
	...
	pub fn has_animation(&mut self, name: &str) -> bool {
		return unsafe { model_has_animation(self.raw(), crate::dora::from_string(name)) } != 0;
	}
	...
	pub fn get_node_by_name(&mut self, name: &str) -> crate::dora::Node {
		return crate::dora::Node::from(unsafe { model_get_node_by_name(self.raw(), crate::dora::from_string(name)) }).unwrap();
	}
	pub fn each_node(&mut self, mut func: Box<dyn FnMut(&dyn crate::dora::INode) -> bool>) -> bool {
		...
		return unsafe { model_each_node(self.raw(), func_id, stack_raw) } != 0;
	}
	pub fn new(filename: &str) -> Model {
		return Model { raw: unsafe { model_new(crate::dora::from_string(filename)) } };
	}
	pub fn get_clip_file(filename: &str) -> String {
		return crate::dora::to_string(unsafe { model_get_clip_file(crate::dora::from_string(filename)) });
	}
	pub fn get_looks(filename: &str) -> Vec<String> {
		return crate::dora::Vector::to_str(unsafe { model_get_looks(crate::dora::from_string(filename)) });
	}
	pub fn get_animations(filename: &str) -> Vec<String> {
		return crate::dora::Vector::to_str(unsafe { model_get_animations(crate::dora::from_string(filename)) });
	}
}