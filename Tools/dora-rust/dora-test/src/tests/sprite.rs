...
pub fn test() {
	...
	let windows_flags = ImGuiWindowFlag::NoSavedSettings | ImGuiWindowFlag::NoResize;
	...
	imgui_node.schedule(Box::new(move |_| {
		...
		if ImGui::begin_opts("Sprite", windows_flags) {
			ImGui::text("Sprite (Rust)");
			if ImGui::begin_child_opts("SpriteSetting", &Vec2::new(-1.0, -40.0), BitFlags::default(), BitFlags::default()) {
