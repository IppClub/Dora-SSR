...
2. 第二步：编写游戏代码
    ...
    - 或者安装统一的 [dora-cli](https://github.com/IppClub/Dora-SSR/blob/main/Tools/dora-cli) 工具，在 Rust 项目目录中运行 `uv tool install Dora-SSR/Tools/dora-cli`，然后执行 `dora rust run Hello --host 192.168.3.1`。这里的 IP 地址是 Dora SSR Web IDE 的地址，`Hello` 是 Dora SSR 资源树中的目标目录名。
