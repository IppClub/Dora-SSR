		...
		if self.ptr == "&" or self.ptr == "" then
			return "(tolua_isvaluenil(tolua_S," ..
				narg ..
					",&tolua_err) || !" ..
						is_func ..
							"(tolua_S," .. narg .. ',"' .. _usertype[self.type] .. '"_slice,' .. def .. ",&tolua_err))"
		else
			return "!" ..
				is_func .. "(tolua_S," .. narg .. ',"' .. _usertype[self.type] .. '"_slice,' .. def .. ",&tolua_err)"
		end
	end
end
...
function classDeclaration:builddeclaration(narg, cplusplus)
	...
	elseif self.type:match("^tolua_function_") then
		return "  LuaFunction<" ..
			self.type:match("^tolua_function_([^ ]+)") ..
				(self.ptr == "*" and "*> " or "> ") ..
					self.name .. "(tolua_ref_function(tolua_S," .. tostring(narg) .. "));"
	...
	elseif self.dim == "" and _light_object == self.type then
		...
		return "  " .. self.type .. " " .. self.name .. " = " .. to_func .. "(tolua_S," .. narg .. (def ~= "" and "," .. def or "") .. ");"
	end
...
function classDeclaration:getarray(narg)
	if self.dim ~= "" then
		...
		if (t) then
			...
			output(
				"   if (!tolua_isusertypearray(tolua_S,",
				narg,
				',"',
				type,
				'"_slice,',
		...
		if _light_object ~= type then
			...
			output("   ", self.name .. "[i] = ")
			output("tolua_tofieldlight(tolua_S,", narg, ",i+1", def and "," .. def or "", ");")
		end
		output("   }")
		output("  }")
	end
