 ...
 if not flags.h then
  ...
  output('\n')
	...
	foreach(
		...
		function(n, v)
			if
				_global_classes[v] and _global_classes[v]:check_public_access() and v ~= "tolua_table" and
					not v:match("^tolua_function_[^ ]+$") and
					v ~= "tolua_handler"
			 then
				local hasField = next(_global_classes[v].lnames) ~= nil
				local hasEnum =
					#(_global_classes[v].enums) ~= 0 or (_global_classes[v].n > 0 and not _global_classes[v][1].kind)
