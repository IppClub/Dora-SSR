...
function classFunction:supcode(local_constructor)
	...
	if not light and check_self then
		...
		output(
			'  if (!self) tolua_error(tolua_S,"' ..
				output_error_hook("invalid 'self' in function '%s'", class .. "." .. self.name) .. '", NULL);'
		)
		output("#endif\n")
	end
	...
	elseif class and self.name == "operator&[]" then
		if flags["1"] then -- for compatibility with tolua5 ?
			output("  ", access, "operator[](", self.args[1].name, "-1) = ", self.args[2].name, ";")
		else
			output("  ", access, "operator[](", self.args[1].name, ") = ", self.args[2].name, ";")
		end
	else
		output("  {")
		if self.type ~= "" and self.type ~= "void" then
			if light then
				output("  ", self.mod, self.type, " tolua_ret = ")
			else
		...
		if class and self.name == "new" then
			if self.new_func then
				if out then
					output(self.new_func, "(")
				else
					output(class .. "::" .. self.new_func, "(")
				end
			elseif light then
				output(self.type, "{")
			else
		...
		elseif class then
			if out then
				...
				if self.cast_operator then
					...
					output(access, "operator ", self.mod, self.type, "(")
				else
	...
	if t:inclass() then
		...
		if
		 ...
		 then
			...
			t.ptr = "*"
		elseif
			string.gsub(t.name, "%b<>", "") == "~" .. string.gsub(t.parent.original_name or t.parent.name, "%b<>", "")
		 then
			t.name = "delete"
			t.lname = "delete"
