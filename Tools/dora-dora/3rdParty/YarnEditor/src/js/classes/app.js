...
export var App = function(name, version) {
	...
	this.validateTitle = function() {
		...
		if (
			...
			!RegExp('^[a-zA-Z0-9_]+$', 'i').test(enteredValue)
		) {
			editorTitle.attr('class', 'title title-error');
			editorTitle.attr(
				'title',
	...
	this.editNode = function(node, dontOpenInactive = true) {
		...
		if (self.settings.editorSplit()) {
			self.splitEditor();
			self.workspace.warpToNodeByIdx(node.index());
		}
	...
	this.getTagBeforeCursor = function() {
		...
		if (textBeforeCursor.startsWith('<<') && !textBeforeCursor.endsWith('>>')) {
			if (selectionRange.start.column > 1) {
				return '<<';
			}
		}
	...
	this.trimBodyLinks = function(body) {
		var re = /\<\<\s*jump\s+([a-zA-Z0-9_]+?)\s*\>\>/g;
	...
	this.updateEditorStats = function(writeCurrent = false) {
		...
		data.saveAppStateToLocalStorage(writeCurrent);
	};
