...
export let Node = function(options = {}) {
	...
	this.getLinksInNode = function(node) {
		let links = (node || self)
			.body()
			.match(Utils.getJumpTargetRegex());

		if (links != undefined) {
			let exists = {};

			for (let i = links.length - 1; i >= 0; i--) {
				links[i] = links[i].replace(Utils.getJumpTargetRegex(), '$1');
		...
		app.nodes().forEach(parent => {
			...
			if (parentLinks) {
				if (parentLinks.includes(self.oldTitle)) {
					const re = RegExp('<<\\s*jump\\s+' + Utils.escapeRegExp(self.oldTitle) + '\\s*>>', 'gu');
					let newBody = parent.body().replace(re, '<<jump ' + self.title() + '>>');
					parent.body(newBody);
