export const UI = function(app) {
	...
	this.createSearchMenuLine = function(node, action, rootMenu, match = 'title') {
		...
		p.textContent = `${node.title()} ${match ? `(${match})` : ''}`;
		$(p).addClass(
			'item ' + node.titleStyles[node.colorID()]
		);
		if (action == 'link') {
			if (node.title() !== app.editing().title()) {
				p.addEventListener('click', () => app.insertTextAtCursor('<<jump ' + node.title() + '>>'));
				rootMenu.appendChild(p);
			}
		} else if (action == 'open') {
			p.addEventListener('click', () => app.openNodeByTitle(node.title()));
			p.addEventListener('mouseenter', () => app.workspace.warpToNodeByIdx(app.nodes.indexOf(node)));
			rootMenu.appendChild(p);
		}
	};
	// openNodeListMenu
