	...
	this.initYarn = (
		...
		playtestVariables.forEach(function(variable) {
			...
			if (variable.type === 'number') {
				variables.set(variable.key, Number(value));
				return;
			}
			if (variable.type === 'boolean') {
				variables.set(variable.key, value === true || value === 'true');
				return;
			}
			if (variable.type === 'string') {
				variables.set(variable.key, String(value ?? ''));
				return;
			}
			const numVar = Number.parseFloat(value);
			...
			if (booleanVar !== undefined) {
				...
				variables.set(variable.key, value);
			}
		});
