...
export var Plugins = function(app) {
	...
	const getPluginStore = pluginName => {
		...
		self.pluginStorage = app.plugins.pluginStorage;
		if (!app.plugins.pluginStorage[pluginName]) {
			app.plugins.pluginStorage = {
				...app.plugins.pluginStorage,
				[pluginName]: {},
			};
			self.pluginStorage = app.plugins.pluginStorage;
		}
		return app.plugins.pluginStorage[pluginName];
	};
	const setPluginStore = (pluginName, key, val) => {
		...
		self.pluginStorage = app.plugins.pluginStorage;
		if (app.data?.saveAppStateToLocalStorage) {
	...
	window.addEventListener('yarnLoadedData', e => {
		if (app.data.documentHeader() !== null) {
			...
			if ('pluginStorage' in documentHeader) {
				app.plugins.pluginStorage = documentHeader.pluginStorage || {};
				self.pluginStorage = app.plugins.pluginStorage;
			}
		}
	});
	...
	const createButton = (
		...
		button.innerHTML = `
			<span class="item ${className || ''}" title="${title || ''}" ${
				onClick ? `onclick="app.plugins.${pluginName}.${onClick}"` : ''
		}
			 ${
				 onPointerDown
