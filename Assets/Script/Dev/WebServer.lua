...
relativeToRoot = function(file, root)
	...
	local relative = Path:getRelative(file, root)
	if relative == "" or relative == ".." or relative:sub(1, 3) == "../" or relative:sub(1, 3) == "..\\" then
		return nil
	end
	if relative == "." then
		return ""
...
getProjectRootFromPath = function(target, isDir)
	...
	if isDir then
		if target == Content.writablePath or isProjectRootDir(target) then
			return target
		end
...
gitSummary = function(repoPath)
	...
	local logRes = gitRunSync(repoPath, "log --metadata-only -n 100", nil, 120)
	local logStatus
...
HttpServer:postSchedule("/git/status-files", function(req)
	do
		...
		if _tab_0 then
			...
			if repoPath ~= nil then
				return gitRunSync(repoPath, "status", nil, 120)
			end
		end
	end
	return invalidArguments
	...
	do
		...
		if _tab_0 then
			...
			if body ~= nil then
				...
				return gitRunSync(repoPath, "log --metadata-only -n " .. tostring(limit), nil, 10)
			end
		end
	end
	return invalidArguments
...
HttpServer:post("/agent/vision/asset", function(req)
	do
		local _type_0 = type(req)
		local _tab_0 = "table" == _type_0 or "userdata" == _type_0
		if _tab_0 then
...
HttpServer:post("/agent/session/mode", function(req)
	do
		local _type_0 = type(req)
		local _tab_0 = "table" == _type_0 or "userdata" == _type_0
		if _tab_0 then
			...
			do
				...
				local _type_1 = type(_obj_0)
				if "table" == _type_1 or "userdata" == _type_1 then
					workMode = _obj_0.workMode
				end
			end
			if sessionId ~= nil and workMode ~= nil then
				return AgentSession.setWorkMode(sessionId, workMode)
			end
		end
	end
	return invalidArguments
	...
	do
		...
		if _tab_0 then
			...
			if sessionId ~= nil and prompt ~= nil then
				return AgentSession.sendPrompt(sessionId, prompt, req.body.disabledAgentTools, req.body.workMode, req.body.llmConfigId)
			end
		end
	end
	return invalidArguments
...
HttpServer:post("/agent/session/continue", function(req)
	do
		...
		if _tab_0 then
			...
			if sessionId ~= nil then
				return AgentSession.continuePrompt(sessionId, req.body.disabledAgentTools, req.body.llmConfigId)
			end
		end
	end
	return invalidArguments
end)
HttpServer:post("/agent/session/finish-handoff", function(req)
	do
		local _type_0 = type(req)
		local _tab_0 = "table" == _type_0 or "userdata" == _type_0
		if _tab_0 then
			...
			if sessionId ~= nil then
				return AgentSession.finishSubSessionHandoff(sessionId, req.body.llmConfigId)
			end
		end
	end
	return invalidArguments
	...
	do
		...
		if _tab_0 then
			...
			if sessionId ~= nil and messageId ~= nil and prompt ~= nil then
				return AgentSession.resendPrompt(sessionId, messageId, prompt, req.body.disabledAgentTools, req.body.workMode, req.body.llmConfigId)
			end
		end
	end
	return invalidArguments
end)
HttpServer:post("/agent/session/questionnaire/respond", function(req)
	do
		local _type_0 = type(req)
		local _tab_0 = "table" == _type_0 or "userdata" == _type_0
		if _tab_0 then
			...
			local questionnaireId
			do
				local _obj_0 = req.body
				local _type_1 = type(_obj_0)
				if "table" == _type_1 or "userdata" == _type_1 then
					questionnaireId = _obj_0.questionnaireId
				end
			end
			local answers
			do
				...
				local _type_1 = type(_obj_0)
				if "table" == _type_1 or "userdata" == _type_1 then
					answers = _obj_0.answers
				end
			end
			if sessionId ~= nil and questionnaireId ~= nil and answers ~= nil then
				return AgentSession.respondQuestionnaire(sessionId, questionnaireId, answers, req.body.llmConfigId)
			end
		end
	end
	return invalidArguments
end)
HttpServer:post("/agent/session/questionnaire/cancel", function(req)
	do
		local _type_0 = type(req)
		local _tab_0 = "table" == _type_0 or "userdata" == _type_0
		if _tab_0 then
			...
			if sessionId ~= nil and questionnaireId ~= nil then
				return AgentSession.cancelQuestionnaire(sessionId, questionnaireId, req.body.llmConfigId)
			end
		end
	end
	return invalidArguments
		...
		if _tab_0 then
			...
			if sessionId ~= nil then
				...
				return {
					...
					pendingQuestionnaire = res.pendingQuestionnaire,
					hasActivePlan = res.hasActivePlan
				}
			end
		end
	end
	...
	do
		...
		if _tab_0 then
			...
			if sessionId ~= nil then
				...
				local access = AgentSession.validateTaskAccess(sessionId, taskId)
	...
	do
		...
		if _tab_0 then
			...
			if sessionId ~= nil and checkpointId ~= nil then
				...
				local access = AgentSession.validateCheckpointAccess(sessionId, checkpointId)
				if not access.success then
					return access
				end
	...
	do
		...
		if _tab_0 then
			...
			if sessionId ~= nil and checkpointId ~= nil then
				...
				local rollbackRes = AgentTools.rollbackCheckpoint(checkpointId, access.session.projectRoot)
	...
	do
		...
		if _tab_0 then
			...
			if sessionId ~= nil and taskId ~= nil then
				...
				local rollbackRes = AgentTools.rollbackTaskChangeSet(taskId, access.session.projectRoot)
				if not rollbackRes.success then
					return rollbackRes
				end
				return {
...
getParamDocs = function(signatures)
	if not (signatures and #signatures > 0) then
		return nil
	end
	local docs = { }
	do
	...
	for i, signature in ipairs(signatures) do
		...
		do
			...
			if doc then
				item.doc = doc.doc
				if doc.params then
					item.params = doc.params
					item.desc = "function(" .. tostring(table.concat(_anon_func_3(doc), ', ')) .. ")"
				elseif doc.returnType then
					item.desc = "function()"
				end
				if doc.returnType then
					item.desc = item.desc .. ": " .. tostring(doc.returnType)
				end
			end
		end
				...
				if Content:exist(path) then
					...
					visitAssets = function(path, folder)
						...
						for _index_0 = 1, #dirs do
							...
							if dir:match("^%.") or dir == "node_modules" then
								goto _continue_0
							end
							local current
							if folder == "" then
...
HttpServer:post("/info", function(req)
	local Entry = require("Script.Dev.Entry")
	...
	do
		...
		local _tab_0 = "table" == _type_0 or "userdata" == _type_0
		if _tab_0 then
			local webIDETourCompleted
			do
				local _obj_0 = req.body
				local _type_1 = type(_obj_0)
				if "table" == _type_1 or "userdata" == _type_1 then
					webIDETourCompleted = _obj_0.webIDETourCompleted
				end
			end
			if webIDETourCompleted ~= nil then
				config.webIDETourCompleted = webIDETourCompleted == true
			end
		end
	end
	local webProfiler, drawerWidth, webIDETourCompleted = config.webProfiler, config.drawerWidth, config.webIDETourCompleted
	...
	return {
		...
		webIDETourCompleted = webIDETourCompleted == true
	}
end)
...
HttpServer:post("/llm/list", function()
	...
	if rows and #rows > 0 then
		...
		for _index_0 = 1, #rows do
			...
			local id, name, url, model, key, contextWindow, temperature, maxTokens, reasoningEffort, customOptions, supportsFunctionCalling = _des_0[1], _des_0[2], _des_0[3], _des_0[4], _des_0[5], _des_0[6], _des_0[7], _des_0[8], _des_0[9], _des_0[10], _des_0[11]
			_accum_0[_len_0] = {
				id = id,
				name = name,
				url = url,
...
HttpServer:post("/llm/create", function(req)
	...
	do
		...
		if _tab_0 then
			local body = req.body
			if body ~= nil then
				local name, url, model, key, contextWindow, temperature, maxTokens, reasoningEffort, customOptions, supportsFunctionCalling = body.name, body.url, body.model, body.key, body.contextWindow, body.temperature, body.maxTokens, body.reasoningEffort, body.customOptions, body.supportsFunctionCalling
				local now = os.time()
...
HttpServer:post("/llm/update", function(req)
	...
	do
		...
		if _tab_0 then
			local body = req.body
			if body ~= nil then
				local id, name, url, model, key, contextWindow, temperature, maxTokens, reasoningEffort, customOptions, supportsFunctionCalling = body.id, body.name, body.url, body.model, body.key, body.contextWindow, body.temperature, body.maxTokens, body.reasoningEffort, body.customOptions, body.supportsFunctionCalling
				local now = os.time()
				id = tonumber(id)
				if id == nil then
					return invalidArguments
...
local moveAsset
moveAsset = function(old, new)
	if not (Content:exist(old) and not Content:exist(new)) then
		return false
	end
	...
	if not Content:move(old, new) then
		return false
	end
	if renamedDir then
...
local normalizeAssetPaths
normalizeAssetPaths = function(paths)
	if not (type(paths) == "table") then
		return nil
	end
	local unique = { }
	...
	for _index_0 = 1, #paths do
		...
		local relative = relativeToRoot(path, Content.writablePath)
		if relative == nil or relative == "" or not Content:exist(path) then
			return nil
		end
	...
	for _index_0 = 1, #candidates do
		...
		for _index_1 = 1, #result do
			local parent = result[_index_1]
			if relativeToRoot(path, parent) ~= nil then
				contained = true
				break
			end
		end
...
HttpServer:postSchedule("/assets/batch", function(req)
	do
		local _type_0 = type(req)
		local _tab_0 = "table" == _type_0 or "userdata" == _type_0
		if _tab_0 then
			...
			do
				...
				local _type_1 = type(_obj_0)
				if "table" == _type_1 or "userdata" == _type_1 then
					sources = _obj_0.sources
				end
			end
			if operation ~= nil and sources ~= nil then
				if not (("delete" == operation or "copy" == operation or "move" == operation)) then
					return {
						success = false,
						message = "invalid operation"
					}
				end
				sources = normalizeAssetPaths(sources)
				...
				if operation ~= "delete" then
					...
					local targetRelative = relativeToRoot(target, Content.writablePath)
					if targetRelative == nil then
						return {
							success = false,
							message = "invalid target"
						}
					end
					if not (Content:exist(target) and Content:isdir(target)) then
						return {
							success = false,
							message = "invalid target"
						}
					...
					for _index_0 = 1, #sources do
						...
						if Content:isdir(source) and relativeToRoot(target, source) ~= nil then
							return {
								success = false,
								message = "target inside source"
							}
						end
						local destination = Path(target, Path:getFilename(source))
						if Content:exist(destination) then
							return {
								success = false,
				...
				local changes = { }
				local affectedSet = { }
				local affectedDirectories = { }
				local addAffected
				addAffected = function(dir)
					if affectedSet[dir] then
						return
					end
					affectedSet[dir] = true
					affectedDirectories[#affectedDirectories + 1] = dir
				end
				if operation ~= "delete" then
					addAffected(target)
				end
				for _index_0 = 1, #sources do
					local source = sources[_index_0]
					addAffected(Path:getPath(source))
					if operation == "delete" then
						if not deleteAsset(source) then
							return {
								success = false,
						...
						if operation == "copy" then
							ok = Content:copyAsync(source, destination)
						else
							ok = moveAsset(source, destination)
						end
						if not ok then
							return {
								success = false,
	...
	do
		...
		if _tab_0 then
			...
			if path ~= nil then
				local readFile
				readFile = function()
					if Content:exist(path) and not Content:isdir(path) then
...
HttpServer:postSchedule("/doc/search", function(req)
	...
	local docType = body.docType
	if not (("dora-tutorial" == docType or "dora-api" == docType or "love-api" == docType or "tic80-api" == docType)) then
		return {
			success = false,
			message = "unsupported doc type"
		}
	end
...
local addGeneratedSourceHeader
addGeneratedSourceHeader = function(codes, language, file, preserveTIC80)
	if preserveTIC80 == nil then
		preserveTIC80 = false
	end
	local header = "-- [" .. tostring(language) .. "]: " .. tostring(file)
	if preserveTIC80 then
	...
	do
		...
		if yueext == _exp_0 then
			...
			yue.compile(inputFile, outputFile, searchPath, function(codes, err, globals)
				...
				resultCodes = addGeneratedSourceHeader(codes, "yue", file, isTIC80)
				return resultCodes
		...
		elseif "tl" == _exp_0 then
			...
			if codes then
				...
				resultCodes = addGeneratedSourceHeader(codes, "tl", file, isTIC80)
...
local visitAssets
visitAssets = function(path, workspace, builtin, recursive)
	...
	for _index_0 = 1, #dirs do
		local dir = dirs[_index_0]
		if workspace then
			if (".upload" == dir or ".download" == dir or ".www" == dir or ".build" == dir or ".git" == dir or ".cache" == dir or "node_modules" == dir) then
				goto _continue_0
			end
		elseif dir == ".git" then
			goto _continue_0
		...
		local dirPath = Path(path, dir)
		if recursive then
			children[#children + 1] = visitAssets(dirPath, workspace, builtin)
		else
			children[#children + 1] = {
				key = dirPath,
				dir = true,
...
HttpServer:post("/assets/children", function(req)
	do
		local _type_0 = type(req)
		local _tab_0 = "table" == _type_0 or "userdata" == _type_0
		if _tab_0 then
			...
			if path ~= nil then
				local workspace, builtin = relativeToRoot(path, Content.writablePath) ~= nil, relativeToRoot(path, Content.assetPath) ~= nil
				if not ((workspace or builtin) and Content:exist(path) and Content:isdir(path)) then
					return {
						success = false
					}
				end
				local node = visitAssets(path, workspace, builtin, false)
				return {
					success = true,
...
HttpServer:post("/assets/files", function(req)
	do
		local _type_0 = type(req)
		local _tab_0 = "table" == _type_0 or "userdata" == _type_0
		if _tab_0 then
			...
			if path ~= nil then
				local workspace = relativeToRoot(path, Content.writablePath) ~= nil
				local builtin = relativeToRoot(path, Content.assetPath) ~= nil
				if not (workspace or builtin) then
					return {
						success = false
					}
				end
				if not (Content:exist(path) and Content:isdir(path)) then
					return {
						success = false
					}
				end
				...
				do
					local _accum_0 = { }
					local _len_0 = 1
					local _list_0 = Content:glob(path, globs, extentionLevels)
					for _index_0 = 1, #_list_0 do
						local file = _list_0[_index_0]
						_accum_0[_len_0] = Path(path, file)
						_len_0 = _len_0 + 1
					end
					files = _accum_0
				end
...
local _anon_func_6 = function(builtinChildren, workspace, zh)
	local _tab_0 = {
		{
	...
	local _obj_0 = workspace.children or { }
...
HttpServer:post("/assets", function()
	local Entry = require("Script.Dev.Entry")
	local engineDev = Entry.getEngineDev()
	local workspace = visitAssets(Content.writablePath, true, nil, false)
	local zh = (App.locale:match("^zh") ~= nil)
	local readme = visitAssets((Path(Content.assetPath, "Doc", zh and "zh-Hans" or "en")), false, true)
	readme.title = zh and "说明文档" or "Readme"
	local apiDoc = visitAssets((Path(Content.assetPath, "Script", "Lib", "Dora", zh and "zh-Hans" or "en")), false, true)
	apiDoc.title = zh and "接口文档" or "API Doc"
	local tools = visitAssets((Path(Content.assetPath, "Script", "Tools")), false, true)
	tools.title = zh and "开发工具" or "Tools"
	local font = visitAssets((Path(Content.assetPath, "Font")), false, true)
	font.title = zh and "字体" or "Font"
	local lib = visitAssets((Path(Content.assetPath, "Script", "Lib")), false, true)
	lib.title = zh and "程序库" or "Lib"
	if engineDev then
		local _list_0 = lib.children
		for _index_0 = 1, #_list_0 do
	...
	if engineDev then
		local dev = visitAssets((Path(Content.assetPath, "Script", "Dev")), false, true)
		do
			local _obj_0 = dev.children
			_obj_0[#_obj_0 + 1] = {
		...
		builtinChildren[#builtinChildren + 1] = dev
	end
	return {
		...
		root = true,
		title = "Assets",
		children = _anon_func_6(builtinChildren, workspace, zh)
	}
end)
HttpServer:post("/entry/list", function(req)
	local Entry = require("Script.Dev.Entry")
	local res = Entry.getLaunchEntries((req and req.body and req.body.refresh == true))
	res.success = true
	return res
end)
...
local collectTSVirtualFiles
collectTSVirtualFiles = function(sourceRoot)
	local files = { }
	local seen = { }
	local addFile
	addFile = function(file, moduleName, virtualFile)
		...
		local targetFile = virtualFile or file
		do
			local entry = seen[targetFile]
			if entry then
		...
		if content then
			...
			seen[targetFile] = entry
			files[#files + 1] = entry
		end
	end
	if sourceRoot and Content:exist(sourceRoot) and Content:isdir(sourceRoot) then
		local _list_0 = Content:glob(sourceRoot, tsSnapshotGlobs)
		for _index_0 = 1, #_list_0 do
			local subFile = _list_0[_index_0]
			addFile(Path(sourceRoot, subFile))
		end
		local libraryRoots = {
			Path(sourceRoot, "Script", "Lib"),
			Path(sourceRoot, "Lib"),
			Path(Content.assetPath, "Script", "Lib")
		}
		for _index_0 = 1, #libraryRoots do
			local libraryRoot = libraryRoots[_index_0]
			if Content:exist(libraryRoot) and Content:isdir(libraryRoot) then
				local _list_1 = Content:glob(libraryRoot, tsSnapshotGlobs)
				for _index_1 = 1, #_list_1 do
					local subFile = _list_1[_index_1]
					local file = Path(libraryRoot, subFile)
					local virtualFile = Path(sourceRoot, subFile)
					addFile(file, nil, virtualFile)
				end
			end
		end
	end
	...
	local declarationRoot = Path(Content.assetPath, "Script", "Lib", "Dora", locale)
	local _list_0 = Content:getFiles(declarationRoot)
	for _index_0 = 1, #_list_0 do
		local file = _list_0[_index_0]
		if Path:getExt(file) == "ts" and Path:getExt(Path:getName(file)) == "d" then
			local fullPath = Path(declarationRoot, file)
			local moduleName = Path:getName(Path:getName(file))
			addFile(fullPath, moduleName)
		end
	end
...
local transpileTSFileWithWebIDE
do
	local tsReadyTimeout <const> = 5
	local tsBuildTimeout <const> = 30
	local tsBuildRequestId = 0
	transpileTSFileWithWebIDE = function(file, content, sourceRoot, files)
		...
		listener:gslot("AppWS", function(event)
			if event.type == "Receive" then
				...
				if res then
					if res.name == "TranspileTSProbe" and res.id == requestId then
						ready = true
					elseif res.name == "TranspileTS" and res.id == requestId then
		...
		emit("AppWS", "Send", json.encode({
			name = "TranspileTSProbe",
			id = requestId
		}))
		local readyDeadline = App.runningTime + tsReadyTimeout
		wait(function()
			return ready or HttpServer.wsConnectionCount == 0 or App.runningTime >= readyDeadline
		end)
		if not ready then
			listener:removeFromParent()
			...
			return {
				success = false,
				file = file,
				message = "TypeScript transpiler is not ready"
			}
		end
		emit("AppWS", "Send", json.encode({
			name = "TranspileTS",
...
local compilerReadyTimeout <const> = 15
local compilerBuildTimeout <const> = 30
local compilerIdleCloseSeconds <const> = 8
local compilerReady = false
local compilerRequested = false
local compilerUsers = 0
local compilerCloseGeneration = 0
local compilerJobId = 0
local compilerJobs = { }
local compilerResults = { }
local compilerInFlight = { }
HttpServer:post("/compiler/ready", function()
	compilerReady = true
	return {
		success = true
	}
end)
HttpServer:post("/compiler/poll", function()
	compilerReady = true
	local job = table.remove(compilerJobs, 1)
	if job then
		compilerInFlight[job.id] = true
	end
	return {
		success = true,
		job = job
...
HttpServer:post("/compiler/result", function(req)
	do
		local _type_0 = type(req)
		local _tab_0 = "table" == _type_0 or "userdata" == _type_0
		if _tab_0 then
			...
			if body ~= nil then
				local id = body.id
				if not (id and compilerInFlight[id]) then
					return {
						success = false,
						message = "invalid compiler job"
					}
				end
				compilerInFlight[id] = nil
				compilerResults[id] = body
				return {
					success = true
				}
			end
...
local acquireCompiler
acquireCompiler = function()
	compilerUsers = compilerUsers + 1
	compilerCloseGeneration = compilerCloseGeneration + 1
	if not compilerRequested then
		compilerRequested = true
		compilerReady = false
		return emit("AppWebView", "/compiler.html", true)
	end
end
local releaseCompiler
releaseCompiler = function()
	compilerUsers = math.max(compilerUsers - 1, 0)
	compilerCloseGeneration = compilerCloseGeneration + 1
	if not (compilerUsers == 0) then
		return
	end
	local closeGeneration = compilerCloseGeneration
	return thread(function()
		local closeDeadline = App.runningTime + compilerIdleCloseSeconds
		wait(function()
			return compilerUsers > 0 or compilerCloseGeneration ~= closeGeneration or HttpServer.wsConnectionCount > 0 or App.runningTime >= closeDeadline
		end)
		if compilerUsers == 0 and compilerCloseGeneration == closeGeneration and compilerRequested then
			compilerRequested = false
			compilerReady = false
			return emit("AppWebView", "/compiler.html", false)
		end
	end)
end
local removeCompilerJob
removeCompilerJob = function(id)
	for index = #compilerJobs, 1, -1 do
		if compilerJobs[index].id == id then
			table.remove(compilerJobs, index)
		end
	end
	compilerInFlight[id] = nil
	compilerResults[id] = nil
end
local transpileTSFileWithCompiler
transpileTSFileWithCompiler = function(file, content, sourceRoot, files, isCancelled)
	acquireCompiler()
	local readyDeadline = App.runningTime + compilerReadyTimeout
	wait(function()
		return compilerReady or App.runningTime >= readyDeadline or (isCancelled and isCancelled())
	end)
	if not compilerReady then
		releaseCompiler()
		...
		return {
			success = false,
			file = file,
			message = "TypeScript compiler WebView is not ready"
		}
	end
	compilerJobId = compilerJobId + 1
	local jobId = compilerJobId
	compilerJobs[#compilerJobs + 1] = {
		id = jobId,
		file = file,
		content = content,
		projectRoot = sourceRoot,
	...
	local deadline = App.runningTime + compilerBuildTimeout
	wait(function()
		return compilerResults[jobId] or App.runningTime >= deadline or (isCancelled and isCancelled())
	end)
	local response = compilerResults[jobId]
	removeCompilerJob(jobId)
	releaseCompiler()
	...
	if response.success then
		local luaFile = Path:replaceExt(file, "lua")
		if Content:save(luaFile, response.luaCode) then
			return {
				success = true,
				file = file
			}
		...
		return {
			success = false,
			file = file,
			message = "failed to save " .. tostring(luaFile)
		}
	end
	return {
		success = false,
		file = file,
		message = response.message or "TypeScript transpile failed"
	}
end
local transpileTSFile
transpileTSFile = function(file, content, sourceRoot, files, isCancelled)
	if App.platform == "Android" and HttpServer.wsConnectionCount == 0 then
		return transpileTSFileWithCompiler(file, content, sourceRoot, files, isCancelled)
	end
			...
			if path ~= nil then
				...
				if not Content:isdir(path) then
					...
					if "d" ~= Path:getExt(Path:getName(path)) then
						local files = collectTSVirtualFiles(sourceRoot or Path:getPath(path))
						messages[#messages + 1] = transpileTSFile(path, content, sourceRoot, files)
					end
					return {
						success = true,
						messages = messages
					...
					local files = collectTSVirtualFiles(sourceRoot or path)
					for file, content in pairs(fileData) do
						if "d" == Path:getExt(Path:getName(file)) then
							goto _continue_0
						end
						messages[#messages + 1] = transpileTSFile(file, content, sourceRoot, files)
						::_continue_0::
					end
					return {
						success = true,
...
local status = { }
_module_0 = status
status.transpileTSFile = transpileTSFile
...
HttpServer:postSchedule("/git/commit-files", function(req)
	do
		local _type_0 = type(req)
		local _tab_0 = "table" == _type_0 or "userdata" == _type_0
		if _tab_0 then
			local body = req.body
			if body ~= nil then
				local repoPath, commit = body.repoPath, body.commit
				if gitInvalidRepoPath(repoPath) then
					return {
						success = false,
						message = "invalid repoPath"
				...
				local res = gitRunSync(repoPath, "log --changed-files " .. tostring(gitQuote(commit)), nil, 10)
				if not res.success then
					return res
				end
				return {
...
thread(function()
	...
	if Content:exist(doraWeb) then
		local heavyAssets = Path(Content.assetPath, "www", "heavy-assets.json")
		local heavyAssetsContent
		if Content:exist(heavyAssets) then
			heavyAssetsContent = Content:load(heavyAssets)
		else
			heavyAssetsContent = ""
		end
