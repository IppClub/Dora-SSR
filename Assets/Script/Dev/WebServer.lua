...
local clampNumber -- 122
clampNumber = function(value, fallback, minValue, maxValue) -- 122
	local number = tonumber(value) -- 123
	if not number then -- 124
		number = fallback -- 124
	end -- 124
	return math.max(minValue, math.min(maxValue, math.floor(number))) -- 125
end -- 122
local applySceneNodeTransform -- 127
applySceneNodeTransform = function(node, item) -- 127
	node.x = tonumber(item.x) or 0 -- 128
	node.y = tonumber(item.y) or 0 -- 129
	node.scaleX = tonumber(item.scaleX) or 1 -- 130
	node.scaleY = tonumber(item.scaleY) or 1 -- 131
	node.angle = tonumber(item.rotation) or 0 -- 132
	node.visible = item.visible ~= false -- 133
	local tag = item.name or item.id or "" -- 134
	node.tag = tostring(tag) -- 135
end -- 127
local createPreviewSceneNode -- 137
createPreviewSceneNode = function(item) -- 137
	local node = nil -- 138
	local _exp_0 = item.type -- 139
	if "Sprite" == _exp_0 then -- 140
		local texture = item.texture -- 141
		if type(texture) == "string" and texture ~= "" then -- 142
			node = Sprite(texture) -- 143
		end -- 142
	elseif "Label" == _exp_0 then -- 144
		local fontSize = clampNumber(item.fontSize, 32, 1, 512) -- 145
		node = Label("sarasa-mono-sc-regular", fontSize) -- 146
		if node then -- 147
			node.text = tostring(item.text or "") -- 147
		end -- 147
	end -- 139
	if not node then -- 148
		node = Node() -- 148
	end -- 148
	applySceneNodeTransform(node, item) -- 149
	return node -- 150
end -- 137
local buildPreviewSceneTree -- 152
buildPreviewSceneTree = function(scene) -- 152
	if not (type(scene) == "table" and type(scene.nodes) == "table") then -- 153
		return Node() -- 153
	end -- 153
	local nodes = { } -- 154
	local root = nil -- 155
	for _index_0 = 1, #scene.nodes do -- 156
		local item = scene.nodes[_index_0] -- 156
		if type(item) == "table" and item.id then -- 157
			local node = createPreviewSceneNode(item) -- 158
			nodes[item.id] = node -- 159
			if item.id == scene.rootId then -- 160
				root = node -- 160
			end -- 160
		end -- 157
	end -- 156
	for _index_0 = 1, #scene.nodes do -- 161
		local item = scene.nodes[_index_0] -- 161
		if type(item) == "table" and item.id and item.parentId then -- 162
			local parent = nodes[item.parentId] -- 163
			local child = nodes[item.id] -- 164
			if parent and child and parent ~= child then -- 165
				parent:addChild(child) -- 165
			end -- 165
		end -- 162
	end -- 161
	return root or Node() -- 166
end -- 152
HttpServer:postSchedule("/scene/render", function(req) -- 168
	local body = type(req) == "table" and req.body or nil -- 169
	if type(body) ~= "table" or type(body.scene) ~= "table" then -- 170
		return { -- 171
			success = false, -- 171
	...
	if not target then -- 177
		return { -- 178
			success = false, -- 178
			message = "failed to create render target" -- 178
		} -- 178
	end -- 177
	target:renderWithClear(root, Color(background), 1, 0) -- 179
	local previewDir = Path(Content.writablePath, ".scene-preview") -- 180
	...
	target:saveAsync(filePath) -- 184
	local saved = Content:exist(filePath) -- 185
	root:removeAllChildren() -- 186
	root:cleanup() -- 187
	Cache:unload(filePath) -- 188
	if not saved then -- 189
		return { -- 190
			success = false, -- 190
			message = "failed to save preview" -- 190
		} -- 190
	end -- 189
	return { -- 192
		success = true, -- 192
