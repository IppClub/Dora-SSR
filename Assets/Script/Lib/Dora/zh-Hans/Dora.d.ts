...
class Label extends Node {
	...
	/**
	 * 用于平滑文本边缘的 SDF 下限和上限阈值，仅适用于 SDF 标签。
	 * 每个分量都会被限制在 0 到 1，并保证下限小于上限。默认使用自动平滑；手动调整时建议使用 0.5 到 0.8 附近的值。
	 */
	smooth: Vec2;
...
interface LabelClass {
	 ...
	 * @param fontStr 用于创建 Label 对象的字体字符串。应该以 "fontName;fontSize;sdf" 的格式表示，其中 `sdf` 应该是 "true" 或 "false"，并且可以省略，默认是 true。
	 ...
	 * @param sdf [可选] 是否启用SDF渲染。启用SDF渲染后，描边功能将生效。(默认是true)
	 * @returns 新的 Label 对象，如果创建失败则返回 `undefined`。
	 */
...
interface VideoNodeClass {
	/**
	 * 创建一个新的 VideoNode 实例。
	 * @param filename 视频文件路径，应为包含 Theora 视频流的 Ogg（`.ogv`）文件。VideoNode 通过 Dora Content 读取，仅渲染视频，支持 Theora 4:2:0/4:2:2/4:4:4 并使用软件解码；音频需单独播放。
...
/** Dora 场景节点，每个实例拥有并运行一个隔离的 Love Lua State。 */
class LoveNode extends Sprite {
	...
	readonly running: boolean;
	/** 销毁旧 Love State，重新读取 boot 文件，并启动一个全新 State。 */
	restart(): boolean;
}
...
interface LoveNodeClass {
	/** 使用 boot Lua 文件、.love 包或 ZIP 包创建 LoveNode。不带扩展名的相对路径也会尝试解析为 `<路径>.love`、`<路径>.zip` 或 `<路径>/main.lua`；加载失败时返回 undefined。 */
...
interface Git {
	 ...
	 * token 认证的 `username` 可省略，默认值为 `"token"`。仅当 LFS URL 与
	 * Git 远程地址同 host 时，自动 LFS 请求才会复用认证信息；LFS 服务端
	 * 返回的 action 专用认证请求头优先。
	 *
	 * @param repoPath 仓库路径。clone 时该路径是父目录。
	 * @param command Git 命令字符串。
	 * @param callback 接收已解码 GitStatus 表的回调函数。
