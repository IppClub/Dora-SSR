...
import { Audio, HttpServer, Path, emit, json } from 'Dora';
...
function reportProgress(hooks: MusicHooks | undefined, value: number): void {
	const progress = math.max(0, math.min(1, value));
	if (progress < 0.1) {
		hooks?.onProgress?.({
			stage: "compose",
			progress: math.min(1, progress / 0.1),
			message: "preparing music arrangement",
		});
	} else if (progress < 0.95) {
		const synthProgress = (progress - 0.1) / 0.85;
		hooks?.onProgress?.({
			stage: "synth",
			progress: math.max(0, math.min(1, synthProgress)),
			message: `rendering music (${math.floor(synthProgress * 100)}%)`,
		});
	} else {
		hooks?.onProgress?.({
			stage: "write",
			progress: math.max(0, math.min(1, (progress - 0.95) / 0.05)),
			message: "writing music assets",
		});
	}
}
 ...
 */
export function defineMusic(definition: MusicDefinition): MusicDefinition {
	return definition;
}
 ...
 */
export function generateMusicAsync(
	projectDir: string,
	definition: MusicDefinition,
	hooks?: MusicHooks
): MusicResult {
	...
	const [encoded, encodeError] = json.encode(request, false, true);
	if (!encoded) {
		return {
			success: false,
			path: definition.output,
			message: `failed to encode music definition: ${encodeError ?? "unknown error"}`,
		};
	}
	const response = MusicGenerator.renderMusicAsync(encoded, value => reportProgress(hooks, value));
	if (typeof response !== "string") {
		return { success: false, path: definition.output, message: "music generator returned an invalid response" };
	}
	const [decoded, decodeError] = json.decode(response);
	if (!decoded || type(decoded) !== "table") {
		return {
			success: false,
			path: definition.output,
			message: `failed to decode music result: ${decodeError ?? "unknown error"}`,
		};
	}
	const result = decoded as MusicResult;
	if (!result.success) {
		return {
			success: false,
			path: result.path ?? definition.output,
			message: result.message,
		};
	}
	for (let i = 0; i < result.files.length; i++) {
		notifyWebIDE(Path(projectDir, result.files[i]));
	}
	return result;
}
