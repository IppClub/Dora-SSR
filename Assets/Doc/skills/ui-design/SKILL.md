...
Signals update only dynamic roots that read them during render. Unmount roots that are no longer needed with `root.unmount()`, and use `onUnmount` for deterministic cleanup when nodes are removed by diffing.
